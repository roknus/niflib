...
struct VertNorm
{
	...
	{
		if(abs(position.x - n.position.x) > 0.001 || abs(position.y - n.position.y) > 0.001 ||
		   abs(position.z - n.position.z) > 0.001)
		{
			return false;
		}
		if(abs(normal.x - n.normal.x) > 0.001 || abs(normal.y - n.normal.y) > 0.001 ||
...
void ComplexShape::Merge(NiAVObject* root)
{
	...
	{
		throw runtime_error(" The ComplexShape::Merge function requies either a NiNode or a "
	...
	for(vector<NiTriBasedGeomRef>::iterator geom = shapes.begin(); geom != shapes.end(); ++geom)
	{
		...
		if(geomData == NULL)
		{
			throw runtime_error("One of the NiTriBasedGeom found by ComplexShape::Merge with a "
		...
		for(unsigned int c = 0; c < shapeColors.size(); ++c)
		{
			...
			{
				if(colors[c_index].r == newColor.r && colors[c_index].g == newColor.g &&
				   colors[c_index].b == newColor.b && colors[c_index].a == newColor.a)
				{
					// Match found, use existing index
		...
		if(shapeUVs.size() != 0 && (niTexingProp != NULL || niTexProp != NULL || bsTexProp != NULL))
		{
			...
			for(size_t i = 0; i < uvSets.size(); ++i)
			{
				...
				{
					throw runtime_error("One of the UV sets specified in the NiTexturingProperty "
		...
		for(unsigned int t = 0; t < shapeTris.size(); ++t)
		{
			...
			for(unsigned int p = 0; p < 3; ++p)
			{
				...
				for(map<unsigned int, unsigned int>::iterator set =
					...
					set != lookUp[tri[p]].uvIndices.end();
					++set)
				{
					TexCoordIndex tci;
		...
		if(skinInst != NULL)
		{
			...
			// Check to see if the skin is actually a dismember skin instance in which case import
			// the partitions too
			if(skinInst->GetType().IsSameType(BSDismemberSkinInstance::TYPE))
			{
				...
				for(int y = 0; y < skin_partition->GetNumPartitions(); y++)
				{
					...
					for(unsigned int z = 0; z < partition_triangles.size(); z++)
					{
						...
						{
							merged_x =
								lookUp[partition_vertex_map[partition_triangles[z].v1]].vertIndex;
							merged_y =
								lookUp[partition_vertex_map[partition_triangles[z].v2]].vertIndex;
							merged_z =
								lookUp[partition_vertex_map[partition_triangles[z].v3]].vertIndex;
						}
						else
						{
						...
						for(; w < faces.size(); w++)
						{
							...
							// keep this commented code is case my theory that all triangles must
							// have vertices arranged in a certain way and that you can't rearrange
							// vertices in a triangle

							/*if(current_face.points[0].vertexIndex == merged_x) {
								if(current_face.points[1].vertexIndex == merged_y &&
							current_face.points[2].vertexIndex == merged_z) { is_same_face = true;
									break;
								} else if(current_face.points[2].vertexIndex == merged_y &&
							current_face.points[1].vertexIndex == merged_z) { is_same_face = true;
									break;
								}
							...
							current_face.points[0].vertexIndex == merged_z) { is_same_face = true;
									break;
								}
							...
							current_face.points[1].vertexIndex == merged_z) { is_same_face = true;
									break;
								} else if(current_face.points[1].vertexIndex == merged_y &&
							current_face.points[0].vertexIndex == merged_z) { is_same_face = true;
									break;
								}
							} */
	...
	{
		for(map<NiNodeRef, float>::iterator w = vns[v].weights.begin(); w != vns[v].weights.end();
			++w)
		{
	...
	for(map<NiNodeRef, unsigned int>::iterator si = boneLookUp.begin(); si != boneLookUp.end();
		++si)
	{
		si->second = si_index;
...
Ref<NiAVObject> ComplexShape::Split(
	NiNode* parent,
	Matrix44& transform,
	int max_bones_per_partition,
	bool stripify,
	bool tangent_space,
	float min_vertex_weight,
	byte tspace_flags) const
{
	...
	if(dismemberPartitionsFaces.size() > 0)
	{
		...
		{
			throw runtime_error("The number of faces mapped to skin partitions is different from "
	...
	for(unsigned int shape_num = 0; shape_num < shapes.size(); ++shape_num)
	{
		...
		for(vector<ComplexFace>::const_iterator face = faces.begin(); face != faces.end(); ++face)
		{
			...
			else
			{
				...
				for(unsigned int i = 0; i < shapeFacePoints.size() - 2; ++i)
				{
					...
					// all the resulting triangles belong in the the same dismember partition or
					// better said skin partition
		...
		// if no face points to a certain dismember partition then that dismember partition must be
		// removed
		...
		// Check if the properties are skyrim specific in which case attach them in the 2 special
		// slots called bs_properties
		if(propGroups.size() > 0)
		{
			BSLightingShaderPropertyRef shader_property = NULL;

			for(vector<NiPropertyRef>::const_iterator prop = propGroups[shape_num].begin();
				prop != propGroups[shape_num].end();
				++prop)
			{
		...
		for(vector<CompoundVertex>::iterator cv = compVerts.begin(); cv != compVerts.end(); ++cv)
		{
			...
			for(list<int>::iterator tex = shapeTexCoordSets.begin(); tex != shapeTexCoordSets.end();
				++tex)
			{
			...
			SkinWeight sk;
			for(map<NiNodeRef, float>::iterator wt = cv->weights.begin(); wt != cv->weights.end();
				++wt)
			{
		...
		if(shapeWeights.size() > 0)
		{
			...
			for(map<NiNodeRef, vector<SkinWeight>>::iterator inf = shapeWeights.begin();
				inf != shapeWeights.end();
				++inf)
			{
			...
			else
			{
				...
				BSDismemberSkinInstanceRef dismember_skin =
					DynamicCast<BSDismemberSkinInstance>(shapes[shape_num]->GetSkinInstance());
			...
			if(use_dismember_partitions == true)
			{
				...
				shapes[shape_num]
					->GenHardwareSkinInfo(max_bones_per_partition, 4, stripify, face_map);
				delete[] face_map;
