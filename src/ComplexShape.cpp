...
struct VertNorm {
  ...
  VertNorm() {}
  ~VertNorm() {}
  VertNorm(const VertNorm &n) { *this = n; }
  ...
  bool operator==(const VertNorm &n) const {
    if (abs(position.x - n.position.x) > 0.001 ||
        abs(position.y - n.position.y) > 0.001 ||
        abs(position.z - n.position.z) > 0.001) {
      return false;
    }
    if (abs(normal.x - n.normal.x) > 0.001 ||
        abs(normal.y - n.normal.y) > 0.001 ||
        abs(normal.z - n.normal.z) > 0.001) {
      return false;
    }
...
struct CompoundVertex {
  ...
  CompoundVertex() {}
  ~CompoundVertex() {}
  CompoundVertex(const CompoundVertex &n) { *this = n; }
  ...
  bool operator==(const CompoundVertex &n) const {
...
struct MergeLookUp {
  ...
  map<unsigned int, unsigned int>
      uvIndices; // TexCoordSet Index, TexCoord Index
};

} // namespace Niflib

void ComplexShape::SetName(const string &n) { name = n; }

void ComplexShape::SetVertices(const vector<WeightedVertex> &n) {
  vertices = n;
}

void ComplexShape::SetColors(const vector<Color4> &n) { colors = n; }

void ComplexShape::SetNormals(const vector<Vector3> &n) { normals = n; }

void ComplexShape::SetTexCoordSets(const vector<TexCoordSet> &n) {
  texCoordSets = n;
}

void ComplexShape::SetFaces(const vector<ComplexFace> &n) { faces = n; }
...
void ComplexShape::SetDismemberPartitionsFaces(
...
void ComplexShape::SetDismemberPartitionsBodyParts(
    const vector<BodyPartList> &value) {
  dismemberPartitionsBodyParts = value;
}

string ComplexShape::GetName() const { return name; }

vector<WeightedVertex> ComplexShape::GetVertices() const { return vertices; }

vector<Color4> ComplexShape::GetColors() const { return colors; }

vector<Vector3> ComplexShape::GetNormals() const { return normals; }

vector<TexCoordSet> ComplexShape::GetTexCoordSets() const {
  return texCoordSets;
}

vector<ComplexFace> ComplexShape::GetFaces() const { return faces; }
...
void ComplexShape::Merge(NiAVObject *root) {
  ...
  if (root == NULL) {
    throw runtime_error(
        "Called ComplexShape::Merge with a null root reference.");
  }
  ...
  if (root->IsDerivedType(NiTriBasedGeom::TYPE)) {
    ...
    if (shapes.size() == 0) {
      throw runtime_error(
          "The NiNode passed to ComplexShape::Merge has no shape children.");
    }
  } else {
    throw runtime_error(
        " The ComplexShape::Merge function requies either a NiNode or a "
        "NiTriBasedGeom AVObject.");
  }
  ...
  for (vector<NiTriBasedGeomRef>::iterator geom = shapes.begin();
       geom != shapes.end(); ++geom) {
    ...
    NiTriBasedGeomDataRef geomData =
        DynamicCast<NiTriBasedGeomData>((*geom)->GetData());

    if (geomData == NULL) {
      throw runtime_error(
          "One of the NiTriBasedGeom found by ComplexShape::Merge with a "
          "NiTriBasedGeom has no NiTriBasedGeomData attached.");
    }
    ...
    if ((*geom)->GetSkinInstance() != NULL) {
      ...
      if ((*geom)->GetSkinInstance()->GetType().IsSameType(
              BSDismemberSkinInstance::TYPE)) {
        BSDismemberSkinInstanceRef dismember_skin =
            DynamicCast<BSDismemberSkinInstance>((*geom)->GetSkinInstance());
    ...
    for (unsigned int c = 0; c < shapeColors.size(); ++c) {
      ...
      for (unsigned int c_index = 0; c_index < colors.size(); ++c_index) {
        if (colors[c_index].r == newColor.r &&
            colors[c_index].g == newColor.g &&
            colors[c_index].b == newColor.b &&
            colors[c_index].a == newColor.a) {
          // Match found, use existing index
    ...
    NiPropertyRef niProp =
        (*geom)->GetPropertyByType(NiTexturingProperty::TYPE);
    ...
    if (niProp != NULL &&
        niProp->GetType().IsSameType(BSLightingShaderProperty::TYPE)) {
      BSLightingShaderPropertyRef bs_shader =
          DynamicCast<BSLightingShaderProperty>(niProp);
    ...
    if (shapeUVs.size() != 0 &&
        (niTexingProp != NULL || niTexProp != NULL || bsTexProp != NULL)) {
      if (niTexingProp != NULL) {
        // Add the UV set to the list for every type of texture slot that uses
        // it
        for (int tex = 0; tex < 8; ++tex) {
          if (niTexingProp->HasTexture(tex) == true) {
            ...
            uvSets.push_back(
                pair<TexType, unsigned int>(TexType(tex), td.uvSet));
          }
        }
      ...
      for (size_t i = 0; i < uvSets.size(); ++i) {
        ...
        for (unsigned int set_index = 0; set_index < texCoordSets.size();
             ++set_index) {
        ...
        if (set >= shapeUVs.size() || set < 0) {
          throw runtime_error(
              "One of the UV sets specified in the NiTexturingProperty "
              "did not exist in the NiTriBasedGeomData.");
        }
        for (unsigned int v = 0; v < shapeUVs[set].size(); ++v) {
          ...
          bool match_found = false;
          for (unsigned int tc_index = 0;
               tc_index < texCoordSets[uvSetIndex].texCoords.size();
               ++tc_index) {
          ...
          if (match_found == false) {
            ...
            // Record new index
            lookUp[v].uvIndices[uvSetIndex] =
                (unsigned int)(texCoordSets[uvSetIndex].texCoords.size()) - 1;
          }
        }
      }
    }
    ...
    for (unsigned int t = 0; t < shapeTris.size(); ++t) {
      ...
      for (unsigned int p = 0; p < 3; ++p) {
        ...
        for (map<unsigned int, unsigned int>::iterator set =
                 lookUp[tri[p]].uvIndices.begin();
             set != lookUp[tri[p]].uvIndices.end(); ++set) {
          TexCoordIndex tci;
    ...
    if (skinInst != NULL) {
      ...
      // Check to see if the skin is actually a dismember skin instance in which
      // case import the partitions too
      if (skinInst->GetType().IsSameType(BSDismemberSkinInstance::TYPE)) {
        ...
        for (unsigned int y = 0; y < dismember_skin->GetPartitions().size();
             y++) {
        ...
        for (int y = 0; y < skin_partition->GetNumPartitions(); y++) {
          vector<Triangle> partition_triangles =
              skin_partition->GetTriangles(y);
          vector<unsigned short> partition_vertex_map =
              skin_partition->GetVertexMap(y);
          ...
          for (unsigned int z = 0; z < partition_triangles.size(); z++) {
            ...
            if (has_vertex_map == true) {
              merged_x = lookUp[partition_vertex_map[partition_triangles[z].v1]]
                             .vertIndex;
              merged_y = lookUp[partition_vertex_map[partition_triangles[z].v2]]
                             .vertIndex;
              merged_z = lookUp[partition_vertex_map[partition_triangles[z].v3]]
                             .vertIndex;
            } else {
            ...
            for (; w < faces.size(); w++) {
              ...
              // keep this commented code is case my theory that all triangles
              // must have vertices arranged in a certain way and that you can't
              // rearrange vertices in a triangle

              /*if(current_face.points[0].vertexIndex == merged_x) {
                      if(current_face.points[1].vertexIndex == merged_y &&
              current_face.points[2].vertexIndex == merged_z) { is_same_face =
              true; break; } else if(current_face.points[2].vertexIndex ==
              merged_y && current_face.points[1].vertexIndex == merged_z) {
              is_same_face = true; break;
                      }
              } else if(current_face.points[1].vertexIndex == merged_x) {
                      if(current_face.points[0].vertexIndex == merged_y &&
              current_face.points[2].vertexIndex == merged_z) { is_same_face =
              true; break; } else if(current_face.points[2].vertexIndex ==
              merged_y && current_face.points[0].vertexIndex == merged_z) {
              is_same_face = true; break;
                      }
              } else if(current_face.points[2].vertexIndex == merged_x) {
                      if(current_face.points[0].vertexIndex == merged_y &&
              current_face.points[1].vertexIndex == merged_z) { is_same_face =
              true; break; } else if(current_face.points[1].vertexIndex ==
              merged_y && current_face.points[0].vertexIndex == merged_z) {
              is_same_face = true; break;
                      }
              } */
              ...
              if (current_face.points[0].vertexIndex == merged_x &&
                  current_face.points[1].vertexIndex == merged_y &&
                  current_face.points[2].vertexIndex == merged_z) {
                break;
              }
            }

            if (w - (faces.size() - shapeTris.size()) < shapeTris.size()) {
              current_body_parts_faces[w - (faces.size() - shapeTris.size())] =
                  y;
            }
          }
        }
        ...
        for (unsigned int y = 0; y < current_body_parts.size(); y++) {
          ...
          for (unsigned int z = 0; z < dismemberPartitionsBodyParts.size();
               z++) {
            if (dismemberPartitionsBodyParts[z].bodyPart ==
                    current_body_parts[y].bodyPart &&
                dismemberPartitionsBodyParts[z].partFlag ==
                    current_body_parts[y].partFlag) {
              match_index = z;
              break;
            }
          }
  ...
  for (unsigned int v = 0; v < vns.size(); ++v) {
    for (map<NiNodeRef, float>::iterator w = vns[v].weights.begin();
         w != vns[v].weights.end(); ++w) {
  ...
  for (map<NiNodeRef, unsigned int>::iterator si = boneLookUp.begin();
       si != boneLookUp.end(); ++si) {
    si->second = si_index;
...
Ref<NiAVObject> ComplexShape::Split(NiNode *parent, Matrix44 &transform,
                                    int max_bones_per_partition, bool stripify,
                                    bool tangent_space, float min_vertex_weight,
                                    byte tspace_flags) const {
  ...
  if (dismemberPartitionsFaces.size() > 0) {
    if (dismemberPartitionsFaces.size() != faces.size()) {
      throw runtime_error(
          "The number of faces mapped to skin partitions is different from "
  ...
  for (unsigned int shape_num = 0; shape_num < shapes.size(); ++shape_num) {
    ...
    // a vector that holds in what dismember groups or skin partition does each
    // face belong
    ...
    for (vector<ComplexFace>::const_iterator face = faces.begin();
         face != faces.end(); ++face) {
      ...
      for (vector<ComplexPoint>::const_iterator point = face->points.begin();
           point != face->points.end(); ++point) {
        ...
        if (texCoordSets.size() > 0) {
          for (unsigned int i = 0; i < point->texCoordIndices.size(); ++i) {
            const TexCoordSet &set =
                texCoordSets[point->texCoordIndices[i].texCoordSetIndex];

            cv.texCoords[set.texType] =
                set.texCoords[point->texCoordIndices[i].texCoordIndex];
          }
        }

        bool found_match = false;
        // Search for an identical vertex in the list
        for (unsigned short cv_index = 0; cv_index < compVerts.size();
             ++cv_index) {
      ...
      if (use_dismember_partitions == false) {
        ...
        for (unsigned int i = 0; i < shapeFacePoints.size() - 2; ++i) {
          ...
          // all the resulting triangles belong in the the same dismember
          // partition or better said skin partition
          current_dismember_partitions_faces.push_back(
              dismemberPartitionsFaces[current_face_index]);
        }
      }
      current_face_index++;
    }
    ...
    // if no face points to a certain dismember partition then that dismember
    // partition must be removed
    if (use_dismember_partitions == true) {
      vector<bool> used_dismember_groups(current_dismember_partitions.size(),
                                         false);
      for (unsigned int x = 0; x < current_dismember_partitions_faces.size();
           x++) {
      ...
      for (unsigned int x = 0; x < current_dismember_partitions.size(); x++) {
        if (used_dismember_groups[x] == false) {
          for (unsigned int y = 0;
               y < current_dismember_partitions_faces.size(); y++) {
            if (current_dismember_partitions_faces[y] > x) {
              current_dismember_partitions_faces[y]--;
            }
          }
        } else {
          cleaned_up_dismember_partitions.push_back(
              current_dismember_partitions[x]);
        }
      }
    ...
    // Check if the properties are skyrim specific in which case attach them in
    // the 2 special slots called bs_properties
    if (propGroups.size() > 0) {
      BSLightingShaderPropertyRef shader_property = NULL;

      for (vector<NiPropertyRef>::const_iterator prop =
               propGroups[shape_num].begin();
           prop != propGroups[shape_num].end(); ++prop) {
        NiPropertyRef current_property = *prop;
        if (current_property->GetType().IsSameType(
                BSLightingShaderProperty::TYPE)) {
          shader_property =
              DynamicCast<BSLightingShaderProperty>(current_property);
          break;
        }
      }
    ...
    NiPropertyRef niProp =
        shapes[shape_num]->GetPropertyByType(NiTexturingProperty::TYPE);
    ...
    for (vector<vector<TexCoord>>::iterator set = shapeTCs.begin();
         set != shapeTCs.end(); ++set) {
    ...
    for (vector<CompoundVertex>::iterator cv = compVerts.begin();
         cv != compVerts.end(); ++cv) {
      ...
      for (list<int>::iterator tex = shapeTexCoordSets.begin();
           tex != shapeTexCoordSets.end(); ++tex) {
      ...
      SkinWeight sk;
      for (map<NiNodeRef, float>::iterator wt = cv->weights.begin();
           wt != cv->weights.end(); ++wt) {
    ...
    if (texCoordSets.size() > 0) {
      ...
      for (unsigned int tex_index = 0; tex_index < shapeTCs.size();
           ++tex_index) {
    ...
    if (shapeWeights.size() > 0) {
      ...
      for (map<NiNodeRef, vector<SkinWeight>>::iterator inf =
               shapeWeights.begin();
           inf != shapeWeights.end(); ++inf) {
      ...
      if (use_dismember_partitions == false) {
        ...
        shapes[shape_num]->BindSkinWith(shapeInfluences,
                                        BSDismemberSkinInstance::Create);
        BSDismemberSkinInstanceRef dismember_skin =
            DynamicCast<BSDismemberSkinInstance>(
                shapes[shape_num]->GetSkinInstance());
        dismember_skin->SetPartitions(current_dismember_partitions);
      }

      for (unsigned int inf = 0; inf < shapeInfluences.size(); ++inf) {
        shapes[shape_num]->SetBoneWeights(inf,
                                          shapeWeights[shapeInfluences[inf]]);
      }
      ...
      if (use_dismember_partitions == true) {
        ...
        shapes[shape_num]->GenHardwareSkinInfo(max_bones_per_partition, 4,
                                               stripify, face_map);
        delete[] face_map;
      ...
      //	if ( skinData != NULL ) {
      //		for ( unsigned int inf = 0; inf <
      //shapeInfluences.size(); ++inf ) { 			skinData->SetBoneWeights( inf,
      //shapeWeights[ shapeInfluences[inf] ] );
      //		}
      //	}
      //}
