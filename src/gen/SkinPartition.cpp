...
SkinPartition::SkinPartition()
	...
	, numTriangles((unsigned short)0)
	, numBones((unsigned short)0)
	, numStrips((unsigned short)0)
	, numWeightsPerVertex((unsigned short)0)
	, hasVertexMap(false)
	, hasVertexWeights(false)
	, hasFaces(false)
	, hasBoneIndices(false)
	, unknownShort((unsigned short)0)
	, unknown83C3((unsigned short)0)
	, unknown00001((unsigned short)0)
	, numVertices2((unsigned short)0)
	, unknown00002((unsigned short)0)
	, unknown00003((unsigned short)0)
	, unknown00004((unsigned short)0){};
...
unsigned short SkinPartition::numTrianglesCalc() const
{
	...
	{
		for(vector<unsigned short>::iterator itr = stripLengths.begin(); itr != stripLengths.end();
			++itr)
		{
			len += ((*itr) - 2);
		}
