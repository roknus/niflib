...
void NiPSSimulatorCollidersStep::Read(
	istream& in,
...
void NiPSSimulatorCollidersStep::Write(
	ostream& out,
		...
		else
		{
			if(colliders[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(colliders[i1]));
				if(it != link_map.end())
				{
...
void NiPSSimulatorCollidersStep::FixLinks(
