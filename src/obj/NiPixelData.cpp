...
void NiPixelData::Reset(int new_width, int new_height, PixelFormat px_fmt)
{
	...
	{
		throw runtime_error(
			"Texture height must be a power of two.  1, 2, 4, 8, 16, 32, 64, 256, 512, etc.");
	}

	if((new_width & (new_width - 1)) != 0)
	{
		throw runtime_error(
			"Texture width must be a power of two.  1, 2, 4, 8, 16, 32, 64, 256, 512, etc.");
	}
...
vector<Color4> NiPixelData::GetColors() const
{
	...
	default:
		throw runtime_error(
			"The GetColors function only supports the PX_FMT_RGB8 and PX_FMT_RGBA8 pixel formats.");
	}
...
void NiPixelData::SetColors(const vector<Color4>& new_pixels, bool generate_mipmaps)
{
	...
	{
		throw runtime_error(
			"The SetColors function only supports the PX_FMT_RGB8 and PX_FMT_RGBA8 pixel formats.");
	}
	...
	{
		throw runtime_error(
			"The size informatoin has not been set.  Call the IPixelData::Reset() function first.");
	}
	...
	{
		throw runtime_error("You must pass one color for every pixel in the image.  There should "
							"be height * width colors.");
	}
	...
	for(unsigned int i = 0; i < mipmaps.size(); ++i)
	{
...
#ifdef IM_DEBUG
		cout << "Showing mipmap size " << mipmaps[i].width << " x " << mipmaps[i].height << "."
			 << endl;
		...
		case PX_FMT_RGB8:
			...
			// #ifdef IM_DEBUG
			//	cout << "Showing mipmap after being packed  - size " << mipmaps[i].width << " x " <<
			//mipmaps[i].height << "." << endl; 	imdebug("rgb w=%d h=%d %p", mipmaps[i].width,
			//mipmaps[i].height, &map[0] ); 	cin.get(); #endif
			break;
		case PX_FMT_RGBA8:
			for(unsigned int j = 0; j < mipmaps[i].width * mipmaps[i].height; ++j)
			{
			...
			// #ifdef IM_DEBUG
			//	cout << "Showing mipmap after being packed  - size " << mipmaps[i].width << " x " <<
			//mipmaps[i].height << "." << endl; 	imdebug("rgba w=%d h=%d %p", mipmaps[i].width,
			//mipmaps[i].height, &map[0] ); 	cin.get(); #endif
			break;
		case PX_FMT_PAL8:
			throw runtime_error("The SetColors function only supports the PX_FMT_RGB8 and "
								"PX_FMT_RGBA8 pixel formats.");
			break;
		}
	}
}
