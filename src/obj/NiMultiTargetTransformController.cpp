...
const Type NiMultiTargetTransformController::TYPE(
	"NiMultiTargetTransformController",
	&NiInterpController::TYPE);

NiMultiTargetTransformController::NiMultiTargetTransformController()
	: numExtraTargets((unsigned short)0)
{
...
void NiMultiTargetTransformController::Read(
	istream& in,
...
void NiMultiTargetTransformController::Write(
	ostream& out,
		...
		else
		{
			if(extraTargets[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(extraTargets[i1]));
				if(it != link_map.end())
				{
...
void NiMultiTargetTransformController::FixLinks(
...
void NiMultiTargetTransformController::SetExtraTargets(const vector<Ref<NiAVObject>>& value)
{
	...
	extraTargets.erase(
		std::remove(extraTargets.begin(), extraTargets.end(), (NiAVObject*)NULL),
		extraTargets.end());
}
...
bool NiMultiTargetTransformController::ReplaceExtraTarget(
	NiAVObject* newtarget,
	NiAVObject* oldtarget)
{
	vector<NiAVObject*>& targets = extraTargets;
