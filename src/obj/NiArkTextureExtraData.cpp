...
void NiArkTextureExtraData::Write(
	ostream& out,
		...
		else
		{
			if(textures[i1].texturingProperty != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(textures[i1].texturingProperty));
				if(it != link_map.end())
				{
...
void NiArkTextureExtraData::FixLinks(
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < textures.size(); i1++)
	{
		textures[i1].texturingProperty =
			FixLink<NiTexturingProperty>(objects, link_stack, missing_link_stack, info);
	};
