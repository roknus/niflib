	...
	const NifInfo& info) const
{
	...
	if(info.version >= 0x0303000D)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(skinInstance));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if((info.version >= 0x14020007) && (info.userVersion == 12))
	{
		for(unsigned int i2 = 0; i2 < 2; i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(bsProperties[i2]));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
...
void NiGeometry::BindSkinWith(vector<Ref<NiNode>>& bone_nodes, NiObject* (*SkinInstConstructor)())
{
	...
	{
		throw runtime_error(
			"You have attempted to re-bind a skin that is already bound.  Unbind it first.");
	}
...
void NiGeometry::GetSkinDeformation(vector<Vector3>& vertices, vector<Vector3>& normals) const
{
	...
	{
		throw runtime_error(
			"This NiGeometry has no NiGeometryData so there are no vertices to get.");
	}
	...
	{
		throw runtime_error(
			"Skin Data is missing, cannot calculate skin influenced vertex position.");
	}
...
void NiGeometry::ApplyTransforms()
{
	...
	{
		throw runtime_error(
			"Called ApplyTransform on a NiGeometry object that has no NiGeometryData attached.");
	}
...
// Calculate bounding sphere using minimum-volume axis-align bounding box.  Its fast but not a very
// good fit.
static void CalcAxisAlignedBox(
	const vector<SkinWeight>& n,
...
static void CalcCenteredSphere(
	const vector<SkinWeight>& n,
	const vector<Vector3>& vertices,
	Vector3& center,
	float& radius)
...
void NiGeometry::SetBoneWeights(unsigned int bone_index, const vector<SkinWeight>& n)
{
	...
	{
		throw runtime_error(
			"You must bind a skin before setting vertex weights.  No NiSkinInstance found.");
	}
	...
	{
		throw runtime_error(
			"You must bind a skin before setting vertex weights.  No NiSkinData found.");
	}
