...
void NiControllerSequence::Read(istream& in, list<unsigned int>& link_stack, const NifInfo& info)
{
	...
	if((info.version >= 0x14020007) &&
	   (((info.userVersion >= 11) && ((info.userVersion2 >= 24) && (info.userVersion2 <= 28)))))
	{
		NifStream(block_num, in, info);
		link_stack.push_back(block_num);
	};
...
void NiControllerSequence::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(textKeys != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(textKeys));
				if(it != link_map.end())
				{
	...
	if(info.version >= 0x0A01006A)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(manager));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if((info.version >= 0x0A020000) && (info.version <= 0x14000005))
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(stringPalette));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	   ...
	   (((info.userVersion >= 11) && ((info.userVersion2 >= 24) && (info.userVersion2 <= 28)))))
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(animNotes));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void NiControllerSequence::FixLinks(
...
void NiControllerSequence::AddInterpolator(NiSingleInterpController* obj, byte priority)
{
	...
	if(interp == NULL)
	{
		throw runtime_error("Controller must have an interpolator attached to be added to a "
							"NiControllerSequence with the AddInterpolator function.");
	}
...
void NiControllerSequence::AddInterpolator(
...
void NiControllerSequence::AddGenericInterpolator(
...
void NiControllerSequence::ClearControllerData()
{
	...
	throw runtime_error(
		"The AddInterpolator function cannot be implemented until problems in the XML are solved.");
...
void NiControllerSequence::SetControllerData(const vector<ControllerLink>& value)
{
	if(value.size() != controlledBlocks.size())
		throw runtime_error("The SetControllerData requires the ControllerLink array size to match "
							"the existing array.");
