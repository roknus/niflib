...
const Type bhkBallSocketConstraintChain::TYPE(
	"bhkBallSocketConstraintChain",
	&bhkSerializable::TYPE);
...
bhkBallSocketConstraintChain::bhkBallSocketConstraintChain()
	...
	, numLinks((unsigned int)0)
	, numLinks2((unsigned int)0)
...
void bhkBallSocketConstraintChain::Read(
	istream& in,
...
void bhkBallSocketConstraintChain::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(links[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(links[i1]));
				if(it != link_map.end())
				{
	...
	for(unsigned int i1 = 0; i1 < links2.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(links2[i1]));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void bhkBallSocketConstraintChain::FixLinks(
