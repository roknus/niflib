			...
			else
			{
				if(sources[i2] != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(sources[i2]));
					if(it != link_map.end())
					{
	...
	if(info.version <= 0x03010000)
	{
		for(unsigned int i2 = 0; i2 < images.size(); i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(images[i2]));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
...
void NiFlipController::FixLinks(
