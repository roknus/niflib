...
ATextureRenderData::ATextureRenderData()
	...
	, redMask((unsigned int)0)
	, greenMask((unsigned int)0)
	, blueMask((unsigned int)0)
	, alphaMask((unsigned int)0)
	, bitsPerPixel((byte)0)
	...
	, unknownByte1((byte)0)
	, palette(NULL)
	, numMipmaps((unsigned int)0)
	, bytesPerPixel((unsigned int)0)
{
	...
	else
	{
		if(palette != NULL)
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(palette));
			if(it != link_map.end())
			{
...
void ATextureRenderData::FixLinks(
