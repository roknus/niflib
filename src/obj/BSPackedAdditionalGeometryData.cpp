...
void BSPackedAdditionalGeometryData::Read(
	istream& in,
...
void BSPackedAdditionalGeometryData::Write(
	ostream& out,
...
std::string BSPackedAdditionalGeometryData::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < blockInfos.size(); i1++)
	{
		...
		out << "    Num Channel Bytes Per Element:  " << blockInfos[i1].numChannelBytesPerElement
			<< endl;
		out << "    Num Channel Bytes:  " << blockInfos[i1].numChannelBytes << endl;
		out << "    Num Total Bytes Per Element:  " << blockInfos[i1].numTotalBytesPerElement
			<< endl;
	...
	for(unsigned int i1 = 0; i1 < blocks.size(); i1++)
	{
		...
		if(blocks[i1].hasData)
		{
			...
			for(unsigned int i3 = 0; i3 < blocks[i1].blockOffsets.size(); i3++)
			{
				...
				out << "        Block Offsets[" << i3 << "]:  " << blocks[i1].blockOffsets[i3]
					<< endl;
				array_output_count++;
			};
...
void BSPackedAdditionalGeometryData::FixLinks(
