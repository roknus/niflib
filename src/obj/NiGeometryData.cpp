...
NiGeometryData::NiGeometryData()
	...
	, bsMaxVertices((unsigned short)0)
	, keepFlags((byte)0)
	, compressFlags((byte)0)
	, hasVertices(1)
	, numUvSets((unsigned short)0)
	, bsNumUvSets((unsigned short)0)
	, skyrimMaterial((SkyrimHavokMaterial)0)
	, hasNormals(false)
	, radius(0.0f)
...
void NiGeometryData::Read(istream& in, list<unsigned int>& link_stack, const NifInfo& info)
{
	...
	if(((info.version >= 0x0A000100) &&
		(!((info.version >= 0x14020007) && (info.userVersion >= 11)))))
	{
		...
		else
		{
			if(additionalData != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(additionalData));
				if(it != link_map.end())
				{
	...
	const NifInfo& info)
{
	...
	{
		additionalData =
			FixLink<AbstractAdditionalGeometryData>(objects, link_stack, missing_link_stack, info);
	};
	if((info.version >= 0x14000004) && ((info.userVersion >= 12)))
	{
		if((!IsDerivedType(NiPSysData::TYPE)))
		{
			additionalData = FixLink<AbstractAdditionalGeometryData>(
				objects,
				link_stack,
				missing_link_stack,
				info);
