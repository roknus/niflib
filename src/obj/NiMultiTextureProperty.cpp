...
void NiMultiTextureProperty::Write(
	ostream& out,
			...
			else
			{
				if(textureElements[i1].image != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(textureElements[i1].image));
					if(it != link_map.end())
					{
...
void NiMultiTextureProperty::FixLinks(
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < 5; i1++)
	{
		if(textureElements[i1].hasImage)
		{
			textureElements[i1].image =
				FixLink<NiImage>(objects, link_stack, missing_link_stack, info);
		};
	};
