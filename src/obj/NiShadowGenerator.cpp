...
NiShadowGenerator::NiShadowGenerator()
	: unknownFlags((unsigned short)0)
	, numUnknownLinks1((unsigned int)0)
	, unkownInt2((int)0)
	, target(NULL)
	, unkownFloat4(0.98f)
	, unkownByte5((byte)0)
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(unknownLinks1[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(unknownLinks1[i1]));
				if(it != link_map.end())
				{
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(target));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
...
void NiShadowGenerator::FixLinks(
