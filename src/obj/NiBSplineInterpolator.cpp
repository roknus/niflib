...
void NiBSplineInterpolator::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
	...
	else
	{
		if(splineData != NULL)
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(splineData));
			if(it != link_map.end())
			{
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(basisData));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
...
void NiBSplineInterpolator::FixLinks(
...
**********************************************************************/
void NiBSplineInterpolator::bspline(
	int n,
	int t,
	int l,
	float* control,
