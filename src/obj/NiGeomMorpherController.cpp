...
NiGeomMorpherController::NiGeomMorpherController()
	: extraFlags((unsigned short)0)
	, unknown2((byte)0)
	, data(NULL)
	, alwaysUpdate((byte)0)
	, numInterpolators((unsigned int)0)
	, numUnknownInts((unsigned int)0)
{
...
void NiGeomMorpherController::Write(
	ostream& out,
	...
	if((info.version >= 0x0A01006A) && (info.version <= 0x14000005))
	{
		for(unsigned int i2 = 0; i2 < interpolators.size(); i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(interpolators[i2]));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
	...
	if(info.version >= 0x14010003)
	{
		for(unsigned int i2 = 0; i2 < interpolatorWeights.size(); i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(interpolatorWeights[i2].interpolator));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
...
void NiGeomMorpherController::FixLinks(
	...
	const NifInfo& info)
{
	...
	if(info.version >= 0x14010003)
	{
		for(unsigned int i2 = 0; i2 < interpolatorWeights.size(); i2++)
		{
			interpolatorWeights[i2].interpolator =
				FixLink<NiInterpolator>(objects, link_stack, missing_link_stack, info);
		};
	};
