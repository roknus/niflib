...
NiBoneLODController::NiBoneLODController()
	...
	, numNodeGroups((unsigned int)0)
	, numNodeGroups2((unsigned int)0)
	, numShapeGroups((unsigned int)0)
	, numShapeGroups2((unsigned int)0)
	, unknownInt2((int)0)
	, unknownInt3((int)0)
{
...
void NiBoneLODController::Write(
	ostream& out,
			...
			else
			{
				if(nodeGroups[i1].nodes[i2] != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(nodeGroups[i1].nodes[i2]));
					if(it != link_map.end())
					{
		...
		for(unsigned int i2 = 0; i2 < shapeGroups1.size(); i2++)
		{
			...
			for(unsigned int i3 = 0; i3 < shapeGroups1[i2].linkPairs.size(); i3++)
			{
				...
				else
				{
					if(shapeGroups1[i2].linkPairs[i3].shape != NULL)
					{
						map<NiObjectRef, unsigned int>::const_iterator it = link_map.find(
							StaticCast<NiObject>(shapeGroups1[i2].linkPairs[i3].shape));
						if(it != link_map.end())
						{
							NifStream(it->second, out, info);
							missing_link_stack.push_back(NULL);
				...
				else
				{
					...
					{
						map<NiObjectRef, unsigned int>::const_iterator it = link_map.find(
							StaticCast<NiObject>(shapeGroups1[i2].linkPairs[i3].skinInstance));
						if(it != link_map.end())
						{
							NifStream(it->second, out, info);
							missing_link_stack.push_back(NULL);
						...
						{
							NifStream(0xFFFFFFFF, out, info);
							missing_link_stack.push_back(
								shapeGroups1[i2].linkPairs[i3].skinInstance);
						}
					}
					else
					{
	...
	if((info.version >= 0x04020200) && (info.userVersion == 0))
	{
		for(unsigned int i2 = 0; i2 < shapeGroups2.size(); i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(shapeGroups2[i2]));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
...
void NiBoneLODController::FixLinks(
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < nodeGroups.size(); i1++)
	{
		for(unsigned int i2 = 0; i2 < nodeGroups[i1].nodes.size(); i2++)
		{
			nodeGroups[i1].nodes[i2] =
				FixLink<NiNode>(objects, link_stack, missing_link_stack, info);
		};
	};
	if((info.version >= 0x04020200) && (info.userVersion == 0))
	{
		for(unsigned int i2 = 0; i2 < shapeGroups1.size(); i2++)
		{
			for(unsigned int i3 = 0; i3 < shapeGroups1[i2].linkPairs.size(); i3++)
			{
				shapeGroups1[i2].linkPairs[i3].shape =
					FixLink<NiTriBasedGeom>(objects, link_stack, missing_link_stack, info);
				shapeGroups1[i2].linkPairs[i3].skinInstance =
					FixLink<NiSkinInstance>(objects, link_stack, missing_link_stack, info);
			};
		};
	};
	if((info.version >= 0x0A020000) && (info.version <= 0x0A020000) && (info.userVersion == 1))
...
bool NiBoneLODController::ReplaceShapeInGroup(
	Ref<NiTriBasedGeom> newshape,
	Ref<NiTriBasedGeom> oldshape)
{
	vector<Ref<NiTriBasedGeom>>& shapes = shapeGroups2;
