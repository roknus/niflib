...
NiTexturingProperty::NiTexturingProperty()
	...
	, numShaderTextures((unsigned int)0)
{
...
void NiTexturingProperty::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(baseTexture.source != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(baseTexture.source));
				if(it != link_map.end())
				{
	...
	if(hasDarkTexture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(darkTexture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasDetailTexture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(detailTexture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasGlossTexture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(glossTexture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasGlowTexture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(glowTexture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasBumpMapTexture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(bumpMapTexture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(info.version >= 0x14020007)
	{
		...
		if(hasNormalTexture)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(normalTexture.source));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
		...
		if(hasUnknown2Texture)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(unknown2Texture.source));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
	...
	if(hasDecal0Texture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(decal0Texture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasDecal1Texture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(decal1Texture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasDecal2Texture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(decal2Texture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(hasDecal3Texture)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(decal3Texture.source));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
		...
		for(unsigned int i2 = 0; i2 < shaderTextures.size(); i2++)
		{
			...
			if(shaderTextures[i2].isUsed)
			{
				...
				else
				{
					...
					{
						map<NiObjectRef, unsigned int>::const_iterator it = link_map.find(
							StaticCast<NiObject>(shaderTextures[i2].textureData.source));
						if(it != link_map.end())
						{
							NifStream(it->second, out, info);
							missing_link_stack.push_back(NULL);
...
std::string NiTexturingProperty::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < shaderTextures.size(); i1++)
	{
		...
		if(shaderTextures[i1].isUsed)
		{
			...
			out << "      Has Texture Transform:  "
				<< shaderTextures[i1].textureData.hasTextureTransform << endl;
			if(shaderTextures[i1].textureData.hasTextureTransform)
			{
				out << "        Translation:  " << shaderTextures[i1].textureData.translation
					<< endl;
				out << "        Tiling:  " << shaderTextures[i1].textureData.tiling << endl;
				out << "        W Rotation:  " << shaderTextures[i1].textureData.wRotation << endl;
				out << "        Transform Type?:  " << shaderTextures[i1].textureData.transformType_
					<< endl;
				out << "        Center Offset:  " << shaderTextures[i1].textureData.centerOffset
					<< endl;
			};
...
void NiTexturingProperty::FixLinks(
	...
	const NifInfo& info)
{
	...
	{
		baseTexture.source =
			FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
	};
	if(hasDarkTexture)
	{
		darkTexture.source =
	...
	if(info.version >= 0x14020007)
	{
		...
		if(hasUnknown2Texture)
		{
			unknown2Texture.source =
				FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
		};
	};
	if(hasDecal0Texture)
	{
		decal0Texture.source =
			FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
	};
	if(hasDecal1Texture)
	{
		decal1Texture.source =
			FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
	};
	if(hasDecal2Texture)
	{
		decal2Texture.source =
			FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
	};
	if(hasDecal3Texture)
	{
		decal3Texture.source =
			FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
	};
	if(info.version >= 0x0A000100)
	{
		for(unsigned int i2 = 0; i2 < shaderTextures.size(); i2++)
		{
			if(shaderTextures[i2].isUsed)
			{
				shaderTextures[i2].textureData.source =
					FixLink<NiSourceTexture>(objects, link_stack, missing_link_stack, info);
			};
		};
	};
...
void NiTexturingProperty::SetShaderTexture(int n, TexDesc& new_val)
{
	...
	{
		throw runtime_error(
			"SetShaderTexture - Index out of range.  Call SetShaderTextureCount to resize.");
	}
