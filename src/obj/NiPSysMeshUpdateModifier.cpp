...
void NiPSysMeshUpdateModifier::Read(
	istream& in,
...
void NiPSysMeshUpdateModifier::Write(
	ostream& out,
		...
		else
		{
			if(meshes[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(meshes[i1]));
				if(it != link_map.end())
				{
...
void NiPSysMeshUpdateModifier::FixLinks(
