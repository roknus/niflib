...
NiMesh::NiMesh()
	...
	, unknown55(0.0f)
	, unknown56((int)0)
	, numSubmeshes((unsigned short)0)
	, instancingEnabled(false)
	, numDatas((unsigned int)0)
	, numModifiers((unsigned int)0)
	, unknown100((byte)0)
	, unknown101((int)0)
	, unknown102((unsigned int)0)
	, unknown200((int)0)
	, unknown250((int)0)
	, unknown300((int)0)
	, unknown301((short)0)
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(datas[i1].stream != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(datas[i1].stream));
				if(it != link_map.end())
				{
	...
	for(unsigned int i1 = 0; i1 < modifiers.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(modifiers[i1]));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
std::string NiMesh::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < datas.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < datas[i1].submeshToRegionMap.size(); i2++)
		{
			...
			out << "      Submesh To Region Map[" << i2
				<< "]:  " << datas[i1].submeshToRegionMap[i2] << endl;
			array_output_count++;
		};
	...
	for(unsigned int i1 = 0; i1 < unknown351.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < 10; i2++)
		{
			...
			out << "      Unknown Shorts[" << i2 << "]:  " << unknown351[i1].unknownShorts[i2]
				<< endl;
			array_output_count++;
		};
	};
