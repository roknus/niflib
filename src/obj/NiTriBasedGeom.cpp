...
void NiTriBasedGeom::GenHardwareSkinInfo(
	...
	int max_bones_per_vertex /*= INT_MAX*/,
	bool bStrippify /*= true*/,
	int* face2PartMap /*= NULL*/)
{
	NiSkinPartitionRef skinPart;
	if(max_bones_per_partition == 0) // old method
		skinPart = new NiSkinPartition(this);
	else
		skinPart = new NiSkinPartition(
			this,
...
void NiTriBasedGeom::UpdateTangentSpace(int method)
{
	...
	{
		throw runtime_error("There is no NiTriBasedGeomData attached the NiGeometry upon which "
							"UpdateTangentSpace was called.");
	}
	...
	if(verts.size() != norms.size() || verts.size() != uvs.size() || tris.empty())
	{
