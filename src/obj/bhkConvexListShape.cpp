...
bhkConvexListShape::bhkConvexListShape()
	: numSubShapes((unsigned int)0)
	, material((HavokMaterial)0)
	, skyrimMaterial((SkyrimHavokMaterial)0)
	, unknownFloats(6, (float)0.0, (float)0.0, (float)-0.0, (float)0.0, (float)0.0, (float)-0.0)
	, unknownByte1((byte)0)
	, unknownFloat1(0.0f)
{
		...
		else
		{
			if(subShapes[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(subShapes[i1]));
				if(it != link_map.end())
				{
...
void bhkConvexListShape::FixLinks(
