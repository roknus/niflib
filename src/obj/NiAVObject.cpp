...
NiAVObject::NiAVObject()
	...
	, unknownShort1((unsigned short)8)
	, scale(1.0f)
	...
	const NifInfo& info) const
{
			...
			else
			{
				if(properties[i2] != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(properties[i2]));
					if(it != link_map.end())
					{
	...
	if(info.version >= 0x0A000100)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(collisionObject));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void NiAVObject::SetCollisionObject(NiCollisionObject* value)
{
	if(value != NULL)
	{
		...
		{
			throw runtime_error("You have attempted to add a collision object to a NiAVObject "
								"which is already attached to another NiAVObject.");
		}
