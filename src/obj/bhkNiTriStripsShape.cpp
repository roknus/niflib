...
bhkNiTriStripsShape::bhkNiTriStripsShape()
	...
	, unknownFloat1(0.1f)
	, unknownInt1((unsigned int)0x004ABE60)
	, unknownInt2((unsigned int)1)
	, scale(1.0, 1.0, 1.0)
	, unknownInt3((unsigned int)0)
	, numStripsData((unsigned int)0)
	, numDataLayers((unsigned int)0)
{
...
void bhkNiTriStripsShape::Write(
	ostream& out,
		...
		else
		{
			if(stripsData[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(stripsData[i1]));
				if(it != link_map.end())
				{
...
void bhkNiTriStripsShape::FixLinks(
...
void bhkNiTriStripsShape::CalcMassProperties(
	float density,
	bool solid,
	float& mass,
	float& volume,
	...
	InertiaMatrix& inertia)
{
	...
	for(vector<NiTriStripsDataRef>::iterator itr = stripsData.begin(); itr != stripsData.end();
		++itr)
	{
	...
	Inertia::CalcMassPropertiesPolyhedron(
		verts,
		tris,
		density,
		solid,
