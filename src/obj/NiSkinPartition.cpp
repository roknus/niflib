...
NiSkinPartition::NiSkinPartition()
	: numSkinPartitionBlocks((unsigned int)0)
{
...
void NiSkinPartition::Read(istream& in, list<unsigned int>& link_stack, const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < skinPartitionBlocks.size(); i1++)
	{
		...
		if(info.version <= 0x0A000102)
		{
			...
			{
				skinPartitionBlocks[i1].vertexWeights[i3].resize(
					skinPartitionBlocks[i1].numWeightsPerVertex);
				for(unsigned int i4 = 0; i4 < skinPartitionBlocks[i1].vertexWeights[i3].size();
					i4++)
				{
		...
		if(info.version >= 0x0A010000)
		{
			if(skinPartitionBlocks[i1].hasVertexWeights)
			{
				...
				{
					skinPartitionBlocks[i1].vertexWeights[i4].resize(
						skinPartitionBlocks[i1].numWeightsPerVertex);
					for(unsigned int i5 = 0; i5 < skinPartitionBlocks[i1].vertexWeights[i4].size();
						i5++)
					{
		...
		if(info.version <= 0x0A000102)
		{
			if((skinPartitionBlocks[i1].numStrips != 0))
			{
				...
				{
					skinPartitionBlocks[i1].strips[i4].resize(
						skinPartitionBlocks[i1].stripLengths[i4]);
		...
		if(skinPartitionBlocks[i1].hasBoneIndices)
		{
			...
			{
				skinPartitionBlocks[i1].boneIndices[i3].resize(
					skinPartitionBlocks[i1].numWeightsPerVertex);
	...
	const NifInfo& info) const
{
	...
	for(unsigned int i1 = 0; i1 < skinPartitionBlocks.size(); i1++)
	{
		skinPartitionBlocks[i1].numVertices2 =
			(unsigned short)(skinPartitionBlocks[i1].unknownArr1.size());
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].strips.size(); i2++)
			skinPartitionBlocks[i1].stripLengths[i2] =
				(unsigned short)(skinPartitionBlocks[i1].strips[i2].size());
		skinPartitionBlocks[i1].numWeightsPerVertex =
			(unsigned short)((skinPartitionBlocks[i1].vertexWeights.size() > 0)
								 ? skinPartitionBlocks[i1].vertexWeights[0].size()
								 : 0);
		skinPartitionBlocks[i1].numStrips =
			(unsigned short)(skinPartitionBlocks[i1].stripLengths.size());
		...
		skinPartitionBlocks[i1].numVertices =
			(unsigned short)(skinPartitionBlocks[i1].vertexMap.size());
...
std::string NiSkinPartition::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < skinPartitionBlocks.size(); i1++)
	{
		...
		out << "    Num Weights Per Vertex:  " << skinPartitionBlocks[i1].numWeightsPerVertex
			<< endl;
		array_output_count = 0;
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].bones.size(); i2++)
		{
		...
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].vertexMap.size(); i2++)
		{
			...
			out << "      Vertex Map[" << i2 << "]:  " << skinPartitionBlocks[i1].vertexMap[i2]
				<< endl;
			array_output_count++;
		};
		...
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].vertexWeights.size(); i2++)
		{
			...
			for(unsigned int i3 = 0; i3 < skinPartitionBlocks[i1].vertexWeights[i2].size(); i3++)
			{
				...
				out << "        Vertex Weights[" << i3
					<< "]:  " << skinPartitionBlocks[i1].vertexWeights[i2][i3] << endl;
				array_output_count++;
			};
		};
		array_output_count = 0;
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].stripLengths.size(); i2++)
		{
			...
			out << "      Strip Lengths[" << i2
				<< "]:  " << skinPartitionBlocks[i1].stripLengths[i2] << endl;
			array_output_count++;
		};
		...
		if(skinPartitionBlocks[i1].numStrips != 0)
		{
			...
			for(unsigned int i3 = 0; i3 < skinPartitionBlocks[i1].strips.size(); i3++)
			{
				...
				for(unsigned int i4 = 0; i4 < skinPartitionBlocks[i1].stripLengths[i3]; i4++)
				{
					...
					out << "          Strips[" << i4
						<< "]:  " << skinPartitionBlocks[i1].strips[i3][i4] << endl;
					array_output_count++;
				};
			};
		};
		if(skinPartitionBlocks[i1].numStrips == 0)
		{
			...
			for(unsigned int i3 = 0; i3 < skinPartitionBlocks[i1].triangles.size(); i3++)
			{
				...
				out << "        Triangles[" << i3 << "]:  " << skinPartitionBlocks[i1].triangles[i3]
					<< endl;
				array_output_count++;
			};
		};
		...
		if(skinPartitionBlocks[i1].hasBoneIndices)
		{
			...
			for(unsigned int i3 = 0; i3 < skinPartitionBlocks[i1].boneIndices.size(); i3++)
			{
				...
				for(unsigned int i4 = 0; i4 < skinPartitionBlocks[i1].boneIndices[i3].size(); i4++)
				{
					...
					out << "          Bone Indices[" << i4
						<< "]:  " << skinPartitionBlocks[i1].boneIndices[i3][i4] << endl;
					array_output_count++;
				};
			};
		};
		...
		for(unsigned int i2 = 0; i2 < skinPartitionBlocks[i1].unknownArr1.size(); i2++)
		{
			...
			out << "      Unknown Flags:  " << skinPartitionBlocks[i1].unknownArr1[i2].unknownFlags
				<< endl;
...
void NiSkinPartition::SetVertexBoneIndices(
	int partition,
	int vertex,
...
NiSkinPartition::NiSkinPartition(Ref<NiTriBasedGeom> shape)
{
	...
	if(skinInst == NULL)
	{
		throw runtime_error("You must bind a skin before setting generating skin partitions.  No "
							"NiSkinInstance found.");
	}
	...
	{
		throw runtime_error(
			"Attempted to generate a skin partition on a mesh with no geometry data.");
	}
	...
	for(int i = 0; i < totalBones; ++i)
	{
		...
		for(vector<SkinWeight>::const_iterator skinWeight = skinWeights.begin();
			skinWeight != skinWeights.end();
			++skinWeight)
		{
	...
	{
		throw runtime_error(
			"Attempted to generate a skin partition on a mesh with no weights specified.");
	}
	for(int i = 0; i < nv; ++i)
	{
		vertexMap[i] = i;
	...
	int* faceMap)
{
	...
	if(maxBones > maxBonesPerVertex)
	{
		...
		// qWarning() << "reduced" << c << "vertices to" << maxBonesPerVertex << "bone influences
		// (maximum number of bones per vertex was" << maxBones << ")";
	}
	...
	for(Triangles::iterator itr = triangles.begin(); itr != triangles.end(); ++itr)
	{
		...
		do
		{
			...
			if(int(tribones.size()) > maxBonesPerPartition)
			{
				...
				// now remove that bone from all vertices of this triangle and from all matching
				// vertices too
				for(int t = 0; t < 3; t++)
				{
					bool rem = false;
	...
	for(int p = 0; p < int(parts.size()); p++)
	{
		...
		for(int tri = 0; tri < int(triangles.size()); tri++)
		{
			...
			{
				triangles[tri][t] =
					(int)indexOf(vertices.begin(), vertices.end(), triangles[tri][t]);
			}
		}
		...
		for(size_t v = 0; v < vertices.size(); ++v)
		{
			...
			{
				part.boneIndices[v][b] =
					(int(bwl.size()) > b) ? (int)indexOf(bones.begin(), bones.end(), bwl[b].first)
										  : 0;
