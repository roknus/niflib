...
NiDynamicEffect::NiDynamicEffect()
	: switchState(false)
	, numAffectedNodeListPointers((unsigned int)0)
	, numAffectedNodes((unsigned int)0)
{
			...
			else
			{
				if(affectedNodes[i2] != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(affectedNodes[i2]));
					if(it != link_map.end())
					{
...
std::string NiDynamicEffect::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < affectedNodeListPointers.size(); i1++)
	{
		...
		out << "    Affected Node List Pointers[" << i1 << "]:  " << affectedNodeListPointers[i1]
			<< endl;
		array_output_count++;
	};
	array_output_count = 0;
