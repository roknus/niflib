...
void NiDefaultAVObjectPalette::Read(
	istream& in,
...
void NiDefaultAVObjectPalette::Write(
	ostream& out,
		...
		else
		{
			if(objs[i1].avObject != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(objs[i1].avObject));
				if(it != link_map.end())
				{
...
void NiDefaultAVObjectPalette::FixLinks(
...
void NiDefaultAVObjectPalette::ReplaceObj(
	const Ref<NiAVObject> newobj,
	const Ref<NiAVObject> oldobj)
{
	for(vector<AVObject>::iterator itr = objs.begin(); itr != objs.end(); ++itr)
	{
