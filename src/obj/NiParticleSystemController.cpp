...
NiParticleSystemController::NiParticleSystemController()
	...
	, emitStopTime(0.0f)
	, unknownByte((byte)0)
	, oldEmitRate((unsigned int)0)
	, emitRate(0.0f)
	, lifetime(0.0f)
	, lifetimeRandom(0.0f)
	, emitFlags((unsigned short)0)
	, emitter(NULL)
	, unknownShort2_((unsigned short)0)
	, unknownFloat13_(0.0f)
	, unknownInt1_((unsigned int)0)
	, unknownInt2_((unsigned int)0)
	, unknownShort3_((unsigned short)0)
	, particleLifetime(0.0f)
	, particleLink(NULL)
	, particleTimestamp((unsigned int)0)
	, particleUnknownShort((unsigned short)0)
	, particleVertexId((unsigned short)0)
	, numParticles((unsigned short)0)
	, numValid((unsigned short)0)
	, unknownLink(NULL)
	, particleExtra(NULL)
	, unknownLink2(NULL)
	, trailer((byte)0)
...
void NiParticleSystemController::Read(
	istream& in,
...
void NiParticleSystemController::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
	...
	else
	{
		if(emitter != NULL)
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(emitter));
			if(it != link_map.end())
			{
	...
	if(info.version <= 0x03010000)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(particleLink));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	if(info.version >= 0x04000002)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(unknownLink));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(particleExtra));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(unknownLink2));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
	...
	if(info.version <= 0x03010000)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(colorData));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void NiParticleSystemController::FixLinks(
