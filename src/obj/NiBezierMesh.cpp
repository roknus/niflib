...
NiBezierMesh::NiBezierMesh()
	: numBezierTriangles((unsigned int)0)
	, unknown3((unsigned int)0)
	, count1((unsigned short)0)
	, unknown4((unsigned short)0)
	, unknown5((unsigned int)0)
	, unknown6((unsigned int)0)
	, count2((unsigned short)0)
{
		...
		else
		{
			if(bezierTriangle[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(bezierTriangle[i1]));
				if(it != link_map.end())
				{
	...
	const NifInfo& info)
{
	...
	{
		bezierTriangle[i1] =
			FixLink<NiBezierTriangle4>(objects, link_stack, missing_link_stack, info);
	};
