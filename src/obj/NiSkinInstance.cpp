	...
	const NifInfo& info) const
{
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(skeletonRoot));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
	...
	for(unsigned int i1 = 0; i1 < bones.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(bones[i1]));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void NiSkinInstance::BindSkin(NiNode* skeleton_root, vector<Ref<NiNode>> bone_nodes)
{
	...
	for(unsigned int i = 0; i < bone_nodes.size(); ++i)
	{
		...
		{
			throw runtime_error(
				"All bones must be lower than the skeleton root in the scene graph.");
		}
	}
