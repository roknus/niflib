	...
	const NifInfo& info) const
{
		...
		else
		{
			if(unknownRefs1[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(unknownRefs1[i1]));
				if(it != link_map.end())
				{
	...
	for(unsigned int i1 = 0; i1 < transformDests.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(transformDests[i1]));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(propDescription));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
	...
	const NifInfo& info)
{
	...
	{
		transformDests[i1] =
			FixLink<NiPhysXTransformDest>(objects, link_stack, missing_link_stack, info);
	};
