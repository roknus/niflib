...
void NiTextKeyExtraData::FixLinks(
...
vector<Key<string>> NiTextKeyExtraData::GetKeys() const
{
	...
	for(vector<Key<IndexString>>::const_iterator itr = textKeys.begin(); itr != textKeys.end();
		++itr)
	{
		Key<string> key;
		key.time = (*itr).time;
