...
NiSequence::NiSequence()
	...
	, unknownInt4((int)0)
	, unknownInt5((int)0)
	, numControlledBlocks((unsigned int)0)
	...
	for(unsigned int i1 = 0; i1 < controlledBlocks.size(); i1++)
	{
		if(info.version <= 0x0A010000)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(controlledBlocks[i1].controller));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
		...
		if(info.version >= 0x0A01006A)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(controlledBlocks[i1].interpolator));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
		...
		if((info.version >= 0x0A01006A) && (info.version <= 0x0A01006A))
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(controlledBlocks[i1].unknownLink2));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
		...
		if((info.version >= 0x0A020000) && (info.version <= 0x14000005))
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(controlledBlocks[i1].stringPalette));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < controlledBlocks.size(); i1++)
	{
		if(info.version <= 0x0A010000)
		{
			controlledBlocks[i1].controller =
				FixLink<NiTimeController>(objects, link_stack, missing_link_stack, info);
		};
		if(info.version >= 0x0A01006A)
		{
			controlledBlocks[i1].interpolator =
		...
		if((info.version >= 0x0A01006A) && (info.version <= 0x0A01006A))
		{
			controlledBlocks[i1].unknownLink2 =
				FixLink<NiObject>(objects, link_stack, missing_link_stack, info);
		};
		if((info.version >= 0x0A020000) && (info.version <= 0x14000005))
		{
			controlledBlocks[i1].stringPalette =
				FixLink<NiStringPalette>(objects, link_stack, missing_link_stack, info);
		};
	};
