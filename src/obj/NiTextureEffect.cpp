...
NiTextureEffect::NiTextureEffect()
	...
	, unknown((short)0)
	, textureType((EffectType)EFFECT_ENVIRONMENT_MAP)
	, coordinateGenerationType((CoordGenType)CG_SPHERE_MAP)
	, image(NULL)
	, sourceTexture(NULL)
	, clippingPlane((byte)0)
	, unknownVector(1.0, 0.0, 0.0)
	, unknownFloat(0.0f)
	, ps2L((short)0)
	, ps2K((short)-75)
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(image != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(image));
				if(it != link_map.end())
				{
	...
	if(info.version >= 0x04000000)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(sourceTexture));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
