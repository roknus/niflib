...
void BSPSysHavokUpdateModifier::Read(
	istream& in,
...
void BSPSysHavokUpdateModifier::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(nodes[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(nodes[i1]));
				if(it != link_map.end())
				{
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(modifier));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
...
void BSPSysHavokUpdateModifier::FixLinks(
