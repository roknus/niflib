...
void NiMorphWeightsController::Read(
	istream& in,
...
void NiMorphWeightsController::Write(
	ostream& out,
		...
		else
		{
			if(interpolators[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(interpolators[i1]));
				if(it != link_map.end())
				{
...
void NiMorphWeightsController::FixLinks(
