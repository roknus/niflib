...
const Type BSPSysMultiTargetEmitterCtlr::TYPE(
	"BSPSysMultiTargetEmitterCtlr",
	&NiPSysModifierCtlr::TYPE);
...
void BSPSysMultiTargetEmitterCtlr::Read(
	istream& in,
...
void BSPSysMultiTargetEmitterCtlr::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
	...
	if(info.version >= 0x0A020000)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(visibilityInterpolator));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
void BSPSysMultiTargetEmitterCtlr::FixLinks(
