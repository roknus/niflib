	...
	const NifInfo& info) const
{
		...
		else
		{
			if(actorDescs[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(actorDescs[i1]));
				if(it != link_map.end())
				{
	...
	for(unsigned int i1 = 0; i1 < jointDescs.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(jointDescs[i1]));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	for(unsigned int i1 = 0; i1 < materialDescs.size(); i1++)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(materialDescs[i1].materialDesc));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < materialDescs.size(); i1++)
	{
		materialDescs[i1].materialDesc =
			FixLink<NiPhysXMaterialDesc>(objects, link_stack, missing_link_stack, info);
	};
