...
NiParticleMeshModifier::NiParticleMeshModifier()
	: numParticleMeshes((unsigned int)0)
{
...
void NiParticleMeshModifier::Write(
	ostream& out,
		...
		else
		{
			if(particleMeshes[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(particleMeshes[i1]));
				if(it != link_map.end())
				{
...
void NiParticleMeshModifier::FixLinks(
