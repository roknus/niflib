...
NiControllerManager::NiControllerManager()
	: cumulative(false)
	, numControllerSequences((unsigned int)0)
	, objectPalette(NULL)
{
...
void NiControllerManager::Write(
	ostream& out,
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(controllerSequences[i1] != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(controllerSequences[i1]));
				if(it != link_map.end())
				{
	...
	else
	{
		...
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(objectPalette));
			if(it != link_map.end())
			{
				NifStream(it->second, out, info);
				missing_link_stack.push_back(NULL);
...
void NiControllerManager::FixLinks(
	...
	const NifInfo& info)
{
	...
	for(unsigned int i1 = 0; i1 < controllerSequences.size(); i1++)
	{
		controllerSequences[i1] =
			FixLink<NiControllerSequence>(objects, link_stack, missing_link_stack, info);
	};
	objectPalette =
		FixLink<NiDefaultAVObjectPalette>(objects, link_stack, missing_link_stack, info);
...
void NiControllerManager::SetControllerSequences(const vector<Ref<NiControllerSequence>>& value)
{
	...
	for(vector<NiControllerSequenceRef>::iterator it = controllerSequences.begin();
		it != controllerSequences.end();
		++it)
	{
		(*it)->SetParent(this);
	}
