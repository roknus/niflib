...
bhkBreakableConstraint::bhkBreakableConstraint()
	...
	, numEntities2((unsigned int)0)
	, priority2((unsigned int)1)
	, unknownInt2((unsigned int)0)
	, unknownInt3((unsigned int)0)
	, threshold(0.0f)
	, unknownFloat1(0.0f)
	, removeIfBroken((byte)0)
{
...
void bhkBreakableConstraint::Write(
	ostream& out,
			...
			else
			{
				if(entities2[i2] != NULL)
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(entities2[i2]));
					if(it != link_map.end())
					{
...
void bhkBreakableConstraint::FixLinks(
