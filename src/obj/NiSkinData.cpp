		...
		else
		{
			if(skinPartition != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(skinPartition));
				if(it != link_map.end())
				{
...
std::string NiSkinData::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < boneList.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < 13; i2++)
		{
			...
			out << "      Unknown 13 Shorts[" << i2 << "]:  " << boneList[i1].unknown13Shorts[i2]
				<< endl;
			array_output_count++;
		};
...
Matrix44 NiSkinData::GetBoneTransform(unsigned int bone_index) const
{
	...
	{
		throw runtime_error(
			"The specified bone index was larger than the number of bones in this NiSkinData.");
	}

	return Matrix44(
		boneList[bone_index].skinTransform.translation,
...
void NiSkinData::SetBoneWeights(
...
void NiSkinData::ResetOffsets(NiGeometry* owner)
{
	...
	for(unsigned int i = 0; i < boneList.size(); ++i)
	{
		...
		// Store result
		res_mat.Decompose(
			boneList[i].skinTransform.translation,
