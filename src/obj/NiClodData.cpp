...
NiClodData::NiClodData()
	: unknownShorts((unsigned short)0)
	, unknownCount1((unsigned short)0)
	, unknownCount2((unsigned short)0)
	, unknownCount3((unsigned short)0)
	, unknownFloat(0.0f)
...
std::string NiClodData::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < unknownClodShorts1.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < 6; i2++)
		{
			...
			out << "      Unknown Clod Shorts 1[" << i2 << "]:  " << unknownClodShorts1[i1][i2]
				<< endl;
			array_output_count++;
		};
	};
	...
	for(unsigned int i1 = 0; i1 < unknownClodShorts3.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < 6; i2++)
		{
			...
			out << "      Unknown Clod Shorts 3[" << i2 << "]:  " << unknownClodShorts3[i1][i2]
				<< endl;
			array_output_count++;
		};
	};
