...
NiPSysData::NiPSysData()
	: hasUnknownFloats3(false)
	, unknownShort1((unsigned short)0)
	, unknownShort2((unsigned short)0)
	, hasSubtextureOffsetUvs(false)
	, numSubtextureOffsetUvs((unsigned int)0)
	, aspectRatio(0.0f)
...
std::string NiPSysData::asString(bool verbose) const
{
	...
	for(unsigned int i1 = 0; i1 < particleDescriptions.size(); i1++)
	{
		...
		for(unsigned int i2 = 0; i2 < 3; i2++)
		{
			...
			out << "      Unknown Floats 1[" << i2
				<< "]:  " << particleDescriptions[i1].unknownFloats1[i2] << endl;
			array_output_count++;
		};
	...
	if((hasSubtextureOffsetUvs == 1))
	{
		...
		for(unsigned int i2 = 0; i2 < subtextureOffsetUvs.size(); i2++)
		{
			...
			out << "      Subtexture Offset UVs[" << i2 << "]:  " << subtextureOffsetUvs[i2]
				<< endl;
			array_output_count++;
		};
	};
