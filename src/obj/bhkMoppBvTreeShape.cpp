	...
	else
	{
		if(shape != NULL)
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(shape));
			if(it != link_map.end())
			{
...
void bhkMoppBvTreeShape::FixLinks(
...
void bhkMoppBvTreeShape::CalcMassProperties(
	float density,
	bool solid,
	float& mass,
	float& volume,
