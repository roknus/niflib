...
BSLightingShaderProperty::BSLightingShaderProperty()
	: shaderFlags1((SkyrimShaderPropertyFlags1)2185233153)
	, shaderFlags2((SkyrimShaderPropertyFlags2)32801)
	, uvScale(1.0, 1.0)
	, textureSet(NULL)
	, emissiveMultiple(0.0f)
	, textureClampMode((TexClampMode)0)
	, alpha(1.0f)
	, unknownFloat2(0.0f)
	, glossiness(0.0f)
...
void BSLightingShaderProperty::Read(
	istream& in,
...
void BSLightingShaderProperty::Write(
	ostream& out,
	...
	else
	{
		if(textureSet != NULL)
		{
			map<NiObjectRef, unsigned int>::const_iterator it =
				link_map.find(StaticCast<NiObject>(textureSet));
			if(it != link_map.end())
			{
...
std::string BSLightingShaderProperty::asString(bool verbose) const
{
	...
	if((skyrimShaderType == 11))
	{
		...
		out << "    Parallax Inner Layer Texture Scale:  " << parallaxInnerLayerTextureScale
			<< endl;
...
void BSLightingShaderProperty::FixLinks(
