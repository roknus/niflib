...
NiObjectNET::NiObjectNET()
	...
	, oldExtraInternalId((unsigned int)0)
	, unknownByte((byte)0)
	, extraData(NULL)
	...
	const NifInfo& info) const
{
		...
		else
		{
			if(extraData != NULL)
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(extraData));
				if(it != link_map.end())
				{
	...
	if(info.version >= 0x0A000100)
	{
		...
		for(unsigned int i2 = 0; i2 < extraDataList.size(); i2++)
		{
			...
			else
			{
				...
				{
					map<NiObjectRef, unsigned int>::const_iterator it =
						link_map.find(StaticCast<NiObject>(extraDataList[i2]));
					if(it != link_map.end())
					{
						NifStream(it->second, out, info);
						missing_link_stack.push_back(NULL);
	...
	if(info.version >= 0x03000000)
	{
		...
		else
		{
			...
			{
				map<NiObjectRef, unsigned int>::const_iterator it =
					link_map.find(StaticCast<NiObject>(controller));
				if(it != link_map.end())
				{
					NifStream(it->second, out, info);
					missing_link_stack.push_back(NULL);
...
list<Ref<NiExtraData>> NiObjectNET::GetExtraData() const
{
	...
	for(vector<NiExtraDataRef>::const_iterator it = extraDataList.begin();
		it != extraDataList.end();
		++it)
	{
		extras.push_back(*it);
	}
