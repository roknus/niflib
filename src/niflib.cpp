...
/*!
 * Helper function to split off animation from a nif tree. If no animation groups are defined, then
 * both xnif_root and xkf_root will be NULL. \param root_object The root object of the full tree.
 ...
 * \param info A NifInfo structure that contains information such as the version of the NIF file to
 * create.
 */
static void SplitNifTree(
...
vector<NiObjectRef> ReadNifList(istream& in, list<NiObjectRef>& missing_link_stack, NifInfo* info)
{
	...
	*info = header.Read(in);
	...
	while(true)
	{
		...
		if(in.eof())
		{
			errStream << "End of file reached prematurely.  This NIF may be corrupt or improperly "
						 "supported."
					  << endl;
			if(new_obj != NULL)
			{
				...
				errStream << "====[ "
						  << "Object " << i - 1 << " | " << new_obj->GetType().GetTypeName()
						  << " ]====" << endl;
		...
		{
			// From version 5.0.0.1 to version 10.0.1.106  there is a zero byte at the begining of
			// each object
			...
			if(header.version <= VER_10_1_0_106)
			{
				...
				if(checkValue != 0)
				{
					...
					errStream << "Read failue - Bad object position.  Invalid check value:  "
							  << checkValue << endl;
					if(new_obj != NULL)
					{
						errStream << "Last successfuly read object was:  " << endl;
		...
		else
		{
			...
			{
				errStream << "Read failue - Bad object position.  Invalid Type Name Length:  "
						  << objectTypeLength << endl;
				if(new_obj != NULL)
				{
					errStream << "Last successfuly read object was:  " << endl;
					errStream << "====[ "
		...
		if(header.version >= VER_20_3_0_3)
		{
			...
			if(calcobjsize != objsize)
			{
				...
				errStream << "====[ "
						  << "Object " << i << " | " << objectType << " ]====" << endl;
				errStream << "  Start: " << startobjpos << "  Expected Size: " << objsize
						  << "  Read Size: " << calcobjsize << endl;
				errStream << endl;
			}
		}
...
list<NiObjectRef> ResolveMissingLinkStack(NiObject* root, const list<NiObject*>& missing_link_stack)
{
	...
	for(list<NiObject*>::const_iterator obj = missing_link_stack.begin();
		obj != missing_link_stack.end();
		++obj)
	{
	...
	if(txtkey_obj != NULL)
	{
		if(kf_type == KF_MW)
		{
			...
			for(list<NiObjectRef>::iterator it = mgrs.begin(); it != mgrs.end(); ++it)
			{
				...
				for(vector<NiControllerSequenceRef>::iterator itr = seqs.begin(); itr != seqs.end();
					++itr)
				{
			...
			for(list<NiObjectRef>::iterator it = nodes.begin(); it != nodes.end(); ++it)
			{
				...
				for(list<NiTimeControllerRef>::iterator it = controllers.begin();
					it != controllers.end();
					++it)
				{
					...
					else if((*it)->IsDerivedType(NiTransformController::TYPE))
					{
						...
						NiTransformInterpolatorRef interp =
							DynamicCast<NiTransformInterpolator>(trans->GetInterpolator());
						if(interp != NULL)
						{
				...
				{
					node_controllers.push_back(
						pair<NiNodeRef, NiKeyframeControllerRef>(node, key_controller));
				}
			}

			for(list<pair<NiNodeRef, NiKeyframeControllerRef>>::reverse_iterator it =
					node_controllers.rbegin();
				it != node_controllers.rend();
				++it)
			{
				// Add string data
		...
		else if(kf_type == KF_FFVT3R)
		{
			...
			list<NiObjectRef> nodes =
				GetAllObjectsByType(xnif_root, NiMultiTargetTransformController::TYPE);
			for(list<NiObjectRef>::iterator it = nodes.begin(); it != nodes.end(); ++it)
			{
				if(NiMultiTargetTransformControllerRef ctrl =
					   DynamicCast<NiMultiTargetTransformController>(*it))
				{
	...
	NifGame kf_type)
{
	...
	if(export_files == EXPORT_NIF)
		WriteNifTree(
			file_name_path + file_name_base + ".nif",
			root_object,
			info); // simply export the NIF file!
	...
	else if(kf_type == KF_MW)
	{
		if(export_files == EXPORT_NIF_KF)
		{
			...
			if(xnif_root != NULL && !xkf_roots.empty())
			{
				...
				WriteNifTree(
					file_name_path + "x" + file_name_base + ".kf",
					xkf_roots.front(),
					info);
			};
		}
	...
	else if(kf_type == KF_CIV4)
	{
		...
		Kfm kfm; // dummy
		SplitNifTree(root_object, xnif_root, xkf_roots, kfm, kf_type, info);
		if(export_files == EXPORT_NIF || export_files == EXPORT_NIF_KF ||
		   export_files == EXPORT_NIF_KF_MULTI)
		{
		...
		else if(export_files == EXPORT_NIF_KF_MULTI || export_files == EXPORT_KF_MULTI)
		{
			for(list<NiObjectRef>::iterator it = xkf_roots.begin(); it != xkf_roots.end(); ++it)
			{
				...
				if(seq == NULL)
					continue;
				string path = file_name_path + file_name_base + "_" +
							  CreateFileName(seq->GetTargetName()) + "_" +
							  CreateFileName(seq->GetName()) + ".kf";
...
void MergeSceneGraph(map<string, NiNodeRef>& name_map, NiNode* root, NiAVObject* par)
{
	...
	{
		// This object already exists in the original file, so continue on to its children, if it is
		// a NiNode
	...
	for(unsigned int i = 0; i < data.size(); ++i)
	{
		...
		if(name_map.find(node_name) != name_map.end())
		{
			...
			if(data[i].controller != NULL)
			{
				...
				NiObjectRef clone =
					CloneNifTree(StaticCast<NiObject>(data[i].controller), version, user_version);
			...
			else if(data[i].interpolator != NULL)
			{
				...
				if(ctlr == NULL)
				{
					...
					{
						throw runtime_error(
							"Non-NiSingleInterpController controller found in KF file.");
					}
				...
				// found
				NiObjectRef clone =
					CloneNifTree(StaticCast<NiObject>(data[i].interpolator), version, user_version);
...
unsigned int ParseVersionString(string version)
{
	...
	for(int offset = 3; offset >= 0 && start < version.length(); --offset)
	{
		...
		if(end == string::npos)
		{
			if(offset > 0)
			{
				// This version has only one period in it.  Take the rest of the numbers one
				// character at a time.
				len = 1;
			}
			else
...
Ref<NiObject> CloneNifTree(
	NiObject* root,
	unsigned version,
	unsigned user_version,
	NiObject* target_root)
{
