...
void Inertia::CalcMassPropertiesSphere(
	float radius,
	float density,
	bool solid,
	float& mass,
...
void Inertia::CalcMassPropertiesBox(
	Vector3 size,
	float density,
	bool solid,
	float& mass,
	...
	InertiaMatrix& inertia)
{
	...
	{
		mass = density * ((size[0] * size[1] * 2.0f) + (size[0] * size[2] * 2.0f) +
						  (size[1] * size[2] * 2.0f));
...
void Inertia::CalcMassPropertiesCylinder(
	Vector3 startAxis,
	Vector3 endAxis,
	float radius,
	float density,
	...
	InertiaMatrix& inertia)
{
	...
	{
		extCalcMassPropertiesCylinderRoutine(
			startAxis,
			endAxis,
			radius,
			density,
	...
	if(solid)
	{
		...
		inertia[0][0] = inertia[1][1] =
			mass * (3.0f * pow(radius, 3.0f) + pow(height, 2.0f)) / 12.0f;
	...
	else
	{
		mass = density * height * float(M_PI) * pow(radius, 2.0f);
		inertia[0][0] = inertia[1][1] =
			mass * (6.0f * pow(radius, 3.0f) + pow(height, 2.0f)) / 12.0f;
		inertia[2][2] = mass * (pow(radius, 2.0f));
	}
}

void Inertia::CalcMassPropertiesCapsule(
	Vector3 startAxis,
	Vector3 endAxis,
	float radius,
	float density,
	...
	InertiaMatrix& inertia)
{
	...
	{
		extCalcMassPropertiesCapsuleRoutine(
			startAxis,
			endAxis,
			radius,
			density,
	...
	{
		mass = density * (height * float(M_PI) * pow(radius, 2.0f) +
						  (4.0f * float(M_PI) * pow(radius, 3.0f)) / 3.0f);
	...
	{
		mass = density *
			   (height * 2.0f * float(M_PI) * radius + 2.0f * float(M_PI) * pow(radius, 2.0f));
...
// "How to find the inertia tensor (or other mass properties) of a 3D solid body represented by a
// triangle mesh" http://number-none.com/blow/inertia/bb_inertia.doc
//
// David Eberly
...
// solid = False).
void Inertia::CalcMassPropertiesPolyhedron(
	...
	InertiaMatrix& inertia)
{
	if(extCalcMassPropertiesPolyhedronRoutine)
	{
		extCalcMassPropertiesPolyhedronRoutine(
			...
			&vertices[0],
			int(triangles.size()),
			triangles.empty() ? NULL : &triangles[0],
			density,
			solid,
			mass,
			volume,
	...
	Matrix33 covariance_canonical(2.0f, 1.0f, 1.0f, 1.0f, 2.0f, 1.0f, 1.0f, 1.0f, 2.0f);
	...
	for(vector<Triangle>::const_iterator itr = triBegin, end = triEnd; itr != end; ++itr)
	{
		...
		if(solid)
		{
			...
			covariances.push_back(
				transform * covariance_canonical * transform_transposed * determinant);
			// m = det(A) / 6.0
	...
	if(mass < 0.0001f)
	{
		...
		// raise ZeroDivisionError("mass is zero (consider calculating inertia with a lower
		// dimension)") printf("WARNING: mass is zero");
	...
	Matrix44 translate_correction(Matrix33(
		center[0] * center[0],
		center[0] * center[1],
		center[0] * center[2],
		center[1] * center[0],
