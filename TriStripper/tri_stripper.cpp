...
void tri_stripper::InitTriHeap()
{
	...
	// Note: we had to put all of them into the heap before to ensure coherency of the heap_array
	// object
...
{
	// Allow to restore the cache (modified by ExtendTriToStrip) and implicitly reset the cache hit
	// count
...
strip tri_stripper::BackExtendToStrip(size_t Start, triangle_order Order, bool ClockWise)
{
	...
	// Loop while we can further extend the strip
	for(Node = (m_Triangles.begin() + Start); !Cache() || ((Size + 2) < CacheSize()); ++Size)
	{
	...
	// Even though we could discard the first triangle and start from the next counterclockwise
	// triangle, this often leads to more lonely triangles afterward.
	if(ClockWise)
		return strip();
...
inline tri_stripper::const_link_iterator tri_stripper::LinkToNeighbour(
...
inline tri_stripper::const_link_iterator tri_stripper::BackLinkToNeighbour(
...
inline void tri_stripper::AddTriangle(
	const triangle& Tri,
