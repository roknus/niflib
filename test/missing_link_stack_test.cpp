...
BOOST_AUTO_TEST_CASE(missing_link_stack_simple_test)
{
	...
	for(list<NiObject*>::iterator it = missing_link_stack.begin(); it != missing_link_stack.end();
		it++)
	{
		if((*it) != NULL)
		{
...
BOOST_AUTO_TEST_CASE(missing_link_stack_clone_test)
{
	...
	// clone
	NiTriStripsRef new_shape =
		DynamicCast<NiTriStrips>(CloneNifTree(shape, VER_20_0_0_5, 0, root2));
