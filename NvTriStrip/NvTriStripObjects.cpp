	...
	const unsigned short maxIndex)
{
	...
	for(int i = 0; i < numTriangles; i++)
	{
		...
		if(edgeInfo01 == NULL)
		{
			// since one of it's edges isn't in the edge data structure, it can't already exist in
			// the face structure
		...
		else
		{
			...
			{
				// printf("BuildStripifyInfo: > 2 triangles on an edge... uncertain
				// consequences\n");
			}
			else
			{
...
//
void NvStripifier::GetSharedVertices(
	NvFaceInfo* faceA,
	NvFaceInfo* faceB,
	int* vertex0,
	int* vertex1)
...
void NvStripInfo::Build(NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& faceInfos)
{
	...
	while(nextFace != NULL && !IsMarked(nextFace))
	{
		...
		NvFaceInfo* nextNextFace =
			NvStripifier::FindOtherFace(edgeInfos, testnv0, testnv1, nextFace);

		if((nextNextFace == NULL) || (IsMarked(nextNextFace)))
		{
			// uh, oh, we're following a dead end, try swapping
			NvFaceInfo* testNextFace =
				NvStripifier::FindOtherFace(edgeInfos, nv0, testnv1, nextFace);
...
{
	return (
		(size_t(face.m_v0) == index) || (size_t(face.m_v1) == index) ||
		(size_t(face.m_v2) == index));
}
	...
	const unsigned int restartVal)
{
	...
	for(int i = 0; i < nStripCount; i++)
	{
		...
		{
			NvFaceInfo tFirstFace(
				strip->m_faces[0]->m_v0,
	...
	NvFaceInfoVec& outFaceList)
{
	...
	if(in_cacheSize - CACHE_INEFFICIENCY < 1)
		cacheSize = 1;
	else
...
bool NvStripifier::IsDegenerate(
			...
			for(j = 0; j < numTimes; j++)
			{
				...
				while(faceCtr < threshold + (j * threshold) + degenerateCount)
				{
					if(IsDegenerate(allStrips[i]->m_faces[faceCtr]))
					{
						...
						{
							// but, we do need to delete the degenerate, if it's marked fake, to
							// avoid leaking
							if(allStrips[i]->m_faces[faceCtr]->m_bIsFake)
							{
								delete allStrips[i]->m_faces[faceCtr],
									allStrips[i]->m_faces[faceCtr] = NULL;
							}
							++faceCtr;
						}
					}
				...
				*/
				if(j == numTimes - 1) // last time through
				{
					if((numLeftover < 4) && (numLeftover > 0)) // way too small
					{
						...
						{
							IsDegenerate(allStrips[i]->m_faces[faceCtr]) ? ++degenerateCount
																		 : ++ctr;
							currentStrip->m_faces.push_back(allStrips[i]->m_faces[faceCtr++]);
						}
						numLeftover = 0;
					...
					else
					{
						...
						if(allStrips[i]->m_faces[leftOff]->m_bIsFake)
						{
							delete allStrips[i]->m_faces[leftOff],
								allStrips[i]->m_faces[leftOff] = NULL;
						}
		...
		while(1)
		{
			...
			for(size_t i = 0; i < tempStrips2.size(); i++)
			{
				...
				else if(numHits >= bestNumHits)
				{
					...
					{
						int nUnique =
							NvStripifier::GetUniqueVertexInB(strip->m_faces[1], &tFirstFace);
						if(nUnique == tFirstFace.m_v1)
						{
							SWAP(tFirstFace.m_v0, tFirstFace.m_v1);
						}
	...
	int numSamples)
{
	...
	while(!done)
	{
		...
		for(int i = 0; i < numSamples; i++)
		{
			...
			NvStripInfo* strip01 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge01, true),
				stripId++,
				experimentId++);
			...
			NvStripInfo* strip10 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge10, false),
				stripId++,
				experimentId++);
			...
			NvStripInfo* strip12 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge12, true),
				stripId++,
				experimentId++);
			...
			NvStripInfo* strip21 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge21, false),
				stripId++,
				experimentId++);
			...
			NvStripInfo* strip20 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge20, true),
				stripId++,
				experimentId++);
			...
			NvStripInfo* strip02 = new NvStripInfo(
				NvStripStartInfo(nextFace, edge02, false),
				stripId++,
				experimentId++);
...
//
int NvStripifier::CountRemainingTris(
	std::list<NvStripInfo*>::iterator iter,
