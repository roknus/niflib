...
public:
	...
	/*!
	 * Deletes all image data and sets a new size and format in preparation for new data to be
	 * provided. \param new_width The width of the new texture image. \param new_height The height
	 * of the new texture image. \param px_fmt The pixel format of the new texture image. \sa
	 * NiPixelData::GetWidth, NiPixelData::GetHeight
	 */
	NIFLIB_API void Reset(int new_width, int new_height, PixelFormat px_fmt);

	/*!
	 * Retrieves the the pixels of the texture image stored in this object.  This function does not
	 * work on palettized textures. \return A vector containing the colors of each pixel in the
	 * texture image stored in this object, one row after another starting from the bottom of the
	 * image.  The width of the image must be used to interpret them correctly. \sa
	 * NiPixelData::SetColors, NiPixelData::GetWidth
	 */
	NIFLIB_API vector<Color4> GetColors() const;

	/*!
	 * Sets the the pixels of the texture image stored in this object and optionally generates
	 * mipmaps.  This function does not work for palettized textures. \param new_pixels A vector
	 * containing the colors of each new pixel to be set in the texture image stored in this object,
	 * one row after another starting from the botom of the image. \param generate_mipmaps If true,
	 * mipmaps will be generated for the new image and stored in the file. \sa
	 * NiPixelData::GetColors, NiPixelData::GetWidth
	 */
