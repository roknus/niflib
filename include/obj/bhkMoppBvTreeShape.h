...
public:
	...
	/*!
	 * Get the origin for the shape's mopp code in mopp coordinates. This is the minimum of all
	 * vertices in the packed shape along each axis, minus 0.1. \return The origin value in mopp
	 * coordinates.
	 */
	NIFLIB_API Vector3 GetMoppOrigin() const;

	/*!
	 * Sets the origin for the shape's mopp code in mopp coordinates. This is the minimum of all
	 * vertices in the packed shape along each axis, minus 0.1. \param[in] value The origin in mopp
	 * coordinates.
	 */
...
private:
	...
	{
		return (unsigned int)((info.version <= 0x0A000100) ? (oldMoppData.size() + 1)
														   : moppData.size());
	};
