...
public:
	 ...
	 * \param keys A vector containing new Key<Quaternion> data which will replace any existing
	 * data. \sa IKeyframeData::GetQuatRotateKeys, Key
	 */
	NIFLIB_API void SetQuatRotateKeys(const vector<Key<Quaternion>>& keys);

	/*! Retrieves the type of x-rotation interpolation being used.  XYZ data is only used if
	 * rotation type is set to XYZ_ROTATION_KEY. \return The x-rotation key type specifing the type
	 * of interpolation being used. \sa IKeyframeData::SetXRotateType
	 */
	NIFLIB_API KeyType GetXRotateType() const;

	/*! Sets the type of x-rotation interpolation being used.  Does not affect existing key data.
	 * XYZ data is only used if rotation type is set to XYZ_ROTATION_KEY. \param t The new
	 * x-rotation key type specifing the type of interpolation to be used. \sa
	 * IKeyframeData::GetXRotateType
	 */
	NIFLIB_API void SetXRotateType(KeyType t);

	/*! Retrieves the x-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEY. \return A vector containing Key<float> data which specify rotation over
	 * time in the x axis. \sa IKeyframeData::SetXRotateKeys, Key
	 */
	NIFLIB_API vector<Key<float>> GetXRotateKeys() const;

	/*! Sets the x-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEY. \param keys A vector containing new Key<float> data which will replace any
	 * existing data. \sa IKeyframeData::GetXRotateKeys, Key
	 */
	NIFLIB_API void SetXRotateKeys(const vector<Key<float>>& keys);

	/*! Retrieves the type of y-rotation interpolation being used.  XYZ data is only used if
	 * rotation type is set to XYZ_ROTATION_KEY. \return The y-rotation key type specifing the type
	 * of interpolation being used. \sa IKeyframeData::SetYRotateType
	 */
	NIFLIB_API KeyType GetYRotateType() const;

	/*! Sets the type of y-rotation interpolation being used.  Does not affect existing key data.
	 * XYZ data is only used if rotation type is set to XYZ_ROTATION_KEY. \param t The new
	 * y-rotation key type specifing the type of interpolation to be used. \sa
	 * IKeyframeData::GetYRotateType
	 */
	NIFLIB_API void SetYRotateType(KeyType t);

	/*! Retrieves the y-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEY. \return A vector containing Key<float> data which specify rotation over
	 * time in the y axis. \sa IKeyframeData::SetXRotateKeys, Key
	 */
	NIFLIB_API vector<Key<float>> GetYRotateKeys() const;

	/*! Sets the y-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEY. \param keys A vector containing new Key<float> data which will replace any
	 * existing data. \sa IKeyframeData::GetXRotateKeys, Key
	 */
	NIFLIB_API void SetYRotateKeys(const vector<Key<float>>& keys);

	/*! Retrieves the type of z-rotation interpolation being used.  XYZ data is only used if
	 * rotation type is set to XYZ_ROTATION_KEZ. \return The z-rotation key type specifing the type
	 * of interpolation being used. \sa IKeyframeData::SetZRotateType
	 */
	NIFLIB_API KeyType GetZRotateType() const;

	/*! Sets the type of z-rotation interpolation being used.  Does not affect existing key data.
	 * XYZ data is only used if rotation type is set to XYZ_ROTATION_KEZ. \param t The new
	 * z-rotation key type specifing the type of interpolation to be used. \sa
	 * IKeyframeData::GetZRotateType
	 */
	NIFLIB_API void SetZRotateType(KeyType t);

	/*! Retrieves the z-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEZ. \return A vector containing Key<float> data which specify rotation over
	 * time in the z axis. \sa IKeyframeData::SetXRotateKeys, Key
	 */
	NIFLIB_API vector<Key<float>> GetZRotateKeys() const;

	/*! Sets the z-rotation key data.  XYZ data is only used if rotation type is set to
	 * XYZ_ROTATION_KEZ. \param keys A vector containing new Key<float> data which will replace any
	 * existing data. \sa IKeyframeData::GetXRotateKeys, Key
	 */
