...
public:
	 ...
	 * \param[in] version The version number to use in the memory streaming operation.  Default is
	 * the highest version availiable. \param[in] user_version The game-specific version number
	 * extention. \return A cloned copy of this object as a new object.
	 */
	NIFLIB_API Ref<NiObject> Clone(
		unsigned int version = 0xFFFFFFFF,
		unsigned int user_version = 0);
...
template<class T>
Ref<T> StaticCast(NiObject* object)
{
	return (T*)object;
}

template<class T>
Ref<const T> StaticCast(const NiObject* object)
{
	return (const T*)object;
}

template<class T>
Ref<T> DynamicCast(NiObject* object)
{
...
template<class T>
Ref<const T> DynamicCast(const NiObject* object)
{
	if(object && object->IsDerivedType(T::TYPE))
	{
		return (const T*)object;
...
Ref<T> StaticCast(Ref<U>& object)
{
	return object;
}

template<typename T, typename U>
Ref<T> DynamicCast(Ref<U>& object)
{
	return object;
}

template<typename T, typename U>
Ref<T> StaticCast(const Ref<U>& object)
{
	return Ref<T>(object);
}
