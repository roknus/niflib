...
public:
	...
	/*!
	 * Sets the name and reference to the NiTextKeyExtraData object which will be used by this
	 * controller sequence to specify the keyframe labels or "notes." \param[in] txt_key A reference
	 * to the NiTextKeyExtraData object to use. \sa NiTextKeyExtraData
	 */
	NIFLIB_API void SetTextKey(NiTextKeyExtraData* txt_key);

	/*!
	 * Attatches a controler to this KF file for a KF file of version 10.2.0.0 or below.  Versions
	 * above this use interpolators. \param[in] obj A reference to the new NiTimeController to
	 * attach. \sa NiControllerSequence::ClearChildren, NiControllerSequence::AddInterpolator
	 */
	NIFLIB_API void AddController(NiTimeController* obj);

	/*!
	 * Attaches a controler to this KF file for a KF file of version 10.2.0.0 or below.  Versions
	 * above this use interpolators. \param[in] obj A reference to the new NiTimeController to
	 * attach. \sa NiControllerSequence::ClearChildren, NiControllerSequence::AddInterpolator
	 */
	...
	/*!
	 * Attaches an interpolator to this KF file for a KF file of version greater than 10.2.0.0.
	 * Versions below this use controllers. \param[in] obj A reference to the new controller which
	 * has an interpolator to attach. \param[in] priority Used only in Oblivion to set the priority
	 * of one controller over another when the two are merged. \sa
	 * NiControllerSequence::ClearChildren, NiControllerSequence::AddController
	 */
	 ...
	 * Versions below this use controllers. \param[in] obj A reference to the new controller which
	 * has an interpolator to attach. \param[in] priority Used only in Oblivion to set the priority
	 * of one controller over another when the two are merged. \param[in] include_string_pallete
	 * Indicates if the resulting ControllerLinks will hold reference to the NiStringPallete in the
	 * NiControllerSequence \sa NiControllerSequence::ClearChildren,
	 * NiControllerSequence::AddController
	 */
	NIFLIB_API void AddInterpolator(
		NiSingleInterpController* obj,
		byte priority,
		bool include_string_pallete);

	/*!
	 * Attaches a generic interpolator to this KF file for a KF file of version greater
	 * than 10.2.0.0.  Versions below this use controllers. \param[in] interpolator A reference to
	 * the new interpolator to insert into the controllersequence \param[in] target The target
	 * object that the controller which held the interpolator would act on \param[in]
	 * controller_type_name The name of the type of the controller that held the interpolator
	 * \param[in] priority Used only in Oblivion to set the priority of one controller over another
	 * when the two are merged. \param[in] include_string_pallete Indicates if the resulting
	 * ControllerLinks will hold reference to the NiStringPallete in the NiControllerSequence \sa
	 * NiControllerSequence::ClearChildren, NiControllerSequence::AddController
	 */
	NIFLIB_API void AddGenericInterpolator(
		NiInterpolator* interpolator,
		NiObjectNET* target,
		string controller_type_name,
		byte priority = 0,
		bool include_string_pallete = true);
	...
	/*!
	 * Retrieves the data for the controllers or interpolators which are attached to this controller
	 * sequence. \return A vector containing the data for all controllers. \sa
	 * NiControllerSequence::AddController, NiControllerSequence::AddInterpolator,
	 * NiControllerSequence::SetContollerData
	 */
	 ...
	 * sequence. \return A vector containing the data for all controllers. \sa
	 * NiControllerSequence::AddController, NiControllerSequence::AddInterpolator,
	 * NiControllerSequence::GetContollerData
	 */
	NIFLIB_API void SetControllerData(const vector<ControllerLink>& value);

	/*!
	 * Retrieves the text keys, which are tags associated with keyframe times that mark the start
	 * and stop of each sequence, among other things such as the triggering of sound effects.
