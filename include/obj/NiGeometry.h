...
public:
	 ...
	 * ancestor in the scenegraph.  This becomes the skeleton root.
	 */
	NIFLIB_API void BindSkinWith(
		vector<Ref<NiNode>>& bone_nodes,
		NiObject* (*SkinInstConstructor)());

	/*!
	 * Unbinds this geometry from the bones.  This removes the NiSkinInstance and NiSkinData objects
	 * and causes this geometry to stop behaving as a skin.
	 */
	 ...
	 * \param[in] skin The NiSkinInstance object to be used by this geometry node, or NULL if none
	 * is to be used.
	 */
	NIFLIB_API void SetSkinInstance(Ref<NiSkinInstance> skin);

	/*!
	 * Retrieves the geometry data object used by this geometry node, if any.  This contains the
	 * vertices, normals, etc. and can be shared among several geometry nodes. \return The geometry
	 * data object, or NULL if there is none.
	 */
	NIFLIB_API Ref<NiGeometryData> GetData() const;

	/*!
	 * Sets the geometry data object used by this geometry node.  This contains the vertices,
	 * normals, etc. and can be shared among several geometry nodes. \param[in] n The new geometry
	 * data object, or NULL to clear the current one.
	 */
	NIFLIB_API void SetData(NiGeometryData* n);

	/*!
	 * Retrieves the name of the shader used by this geometry node.  The allowable values are
	 * game-dependent. \return The shader name.
	 */
	NIFLIB_API string GetShader() const;

	/*!
	 * Sets the name of the shader used by this geometry node.  The allowable values are
	 * game-dependent. \param[in] n The new shader name.
	 */
	 ...
	 * \param[out] vertices A vector that will be filled with the skin deformed position of the
	 * verticies. \param[out] normals A vector thta will be filled with the skin deformed normal
	 * values.
	 */
	...
	/*
	 * This automatically normalizes all the skin weights for this geometry node if it is bound to
	 * bones as a skin.  In other words, it will guarantee that the weights for all bones on each
	 * vertex will add up to 1.0.  This can be used to correct bad input data.
	 */
