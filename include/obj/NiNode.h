...
public:
	...
	/*!
	 * Adds a child AV Object to this node.  This is a sub-leaf in the scene graph contained in a
	 * NIF file.  Each AV Object can only be the child of one node. \param[in] obj The AV Object to
	 * add as a child of this node.
	 */
	NIFLIB_API void AddChild(Ref<NiAVObject> obj);

	/*!
	 * Removes an AV Object child from this node.  This is a sub-leaf in the scene graph contained
	 * in a NIF file.  Each AV Object can only be the child of one node. The caller is responsible
	 * that the child is no longer weakly linked elsewhere, for instance, as a skin influence.
	 * \param[in] obj The AV Object to remove as a child from this node.
	 */
	NIFLIB_API void RemoveChild(Ref<NiAVObject> obj);

	/*!
	 * Removes all AV Object children from this node.  These are a sub-leafs in the scene graph
	 * contained in a NIF file.  Each AV Object can only be the child of one node. The caller is
	 * responsible that no child is still weakly linked elsewhere, for instance, as a skin
	 * influence.
	 */
	NIFLIB_API void ClearChildren();

	/*!
	 * Retrieves all AV Object children from this node.  These are a sub-leafs in the scene graph
	 * contained in a NIF file.  Each AV Object can only be the child of one node. \return A list of
	 * all the AV Objects that are children of this node in the scene graph.
	 */
	...
	/*!
	 * Adds a dynamic effect to this node.  This is usually a light, but can also be a texture
	 * effect or something else.  Can affect nodes further down the scene graph from this one as
	 * well. \param[in] effect The new dynamic effect to add to this node.
	 */
	NIFLIB_API void AddEffect(NiDynamicEffect* effect);

	/*!
	 * Removes a dynamic effect to this node.  This is usually a light, but can also be a texture
	 * effect or something else.  Can affect nodes further down the scene graph from this one as
	 * well. \param[in] effect The dynamic effect to remove from this node.
	 */
	NIFLIB_API void RemoveEffect(NiDynamicEffect* effect);

	/*!
	 * Removes all dynamic effects from this node.  These is usually lights, but can also be a
	 * texture effects or something else.  Can affect nodes further down the scene graph from this
	 * one as well.
	 */
	NIFLIB_API void ClearEffects();

	/*!
	 * Retrieves all the dynamic effects attached to this node.  This is usually a light, but can
	 * also be a texture effect or something else.  Can affect nodes further down the scene graph
	 * from this one as well. \return The dynamic effects attached to this node.
	 */
	 ...
	 * Should only be called by NiTriBasedGeom.  Adds a new SkinInstance to the specified mesh.  The
	 * bones must be below this node in the scene graph tree
	 */
