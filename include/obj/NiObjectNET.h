...
public:
	...
	/*!
	 * Adds an extra data object to this one.  The way this data is stored changed after
	 * version 10.0.1.0, so the version can optionally be included to specify the old storage
	 * method. \param[in] obj The NiExtraData object to attach to this object. \param[in] version
	 * The way the extra data is arranged internally varies with the NIF version, so if a file is to
	 * be written, it is best to pass the intended version.  The default is 10.0.1.0, which
	 * specifies the newer behavior.
	 */
	...
	/*!
	 * Changes the internal storage method of the extra data in preparation for writing to a file.
	 * This is only necessary if the extra data was added in one way and needs to be output in
	 * another.  This would happen if extra data was loaded from an old file and needed to be
	 * written to a file with a newer version. \param[in] version Specifies the NIF version that the
	 * internal data should be arranged for.
	 */
	...
	/*!
	 * Used to determine whether this object is animated.  In other words, whether it has any
	 * controllers. \return True if the object has controllers, false otherwise.
	 */
	NIFLIB_API bool IsAnimated();

	/*!
	 * Adds a controller to this object.  Controllers allow various properties to be animated over
	 * time. \param[in] obj The controller to add.
	 */
	NIFLIB_API void AddController(NiTimeController* obj);

	/*!
	 * Removes a controller from this object.  Controllers allow various properties to be animated
	 * over time. \param[in] obj The controller to remove.
	 */
