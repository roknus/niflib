...
const char FIX_LINK_INDEX_ERROR[] = "Object index was not found in object map.  This NIF file may "
									"be invalid or imporperly supported.";
const char FIX_LINK_CAST_ERROR[] = "Link could not be cast to required type during file read. This "
								   "NIF file may be invalid or improperly supported.";

template<class T>
Ref<T> FixLink(
	const map<unsigned, NiObjectRef>& objects,
	...
	const NifInfo& info)
{
	...
	if(object == NULL)
	{
		...
		ss << "Type of object with index " << index
		   << " was:  " << it->second->GetType().GetTypeName() << endl;
