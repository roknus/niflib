...
#ifndef PI
#	define PI                                                                                     \
		3.14159265358979323846f // Probably more accurate than a float can be, but it will just be
								// rounded off anyway
#endif
...
struct TexCoord
{
	...
	/*! Copy constructor	*/
	NIFLIB_API TexCoord(const TexCoord& src)
		: u(src.u)
		, v(src.v)
	{}
...
struct Triangle
{
	 ...
	 * \return The value at the given array index by reference so it can be read or set via the
	 * bracket operator.
	 */
...
struct Vector3
{
	 ...
	 */
	NIFLIB_API Vector3(const Vector3& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
	 ...
	 * \return This vector is returned.
	 */
	NIFLIB_API Vector3& operator=(const Vector3& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
...
struct Vector4
{
	 ...
	 */
	NIFLIB_API Vector4(const Vector4& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
	 ...
	 * \param[in] v The Vector3 to construct this one from
	 */
	NIFLIB_API Vector4(const Vector3& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
	 ...
	 * \return This vector is returned.
	 */
	NIFLIB_API Vector4& operator=(const Vector4& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
...
struct Float2
{
	 ...
	 * bracket operator.
	 */
	NIFLIB_API float& operator[](int n) { return data[n]; }
	 ...
	 * bracket operator.
	 */
	NIFLIB_API float operator[](int n) const { return data[n]; }
...
struct Matrix22
{
	...
	Float2 rows[2]; /*!< The two rows of Float2 structures which hold two floating point numbers
					   each. */
	 ...
	 * \return The Float2 structure for the given row index by reference so it can be read or set
	 * via the bracket operator.
	 */
	NIFLIB_API Float2& operator[](int n) { return rows[n]; }
	NIFLIB_API const Float2& operator[](int n) const { return rows[n]; }
	 ...
	 */
	NIFLIB_API Matrix22(float m11, float m12, float m21, float m22)
	{
		rows[0][0] = m11;
		rows[0][1] = m12;
		rows[1][0] = m21;
	 ...
	 * \param[in] m22 The value to set at row 2, column 2.
	 */
	NIFLIB_API void Set(float m11, float m12, float m21, float m22)
	{
		rows[0][0] = m11;
		rows[0][1] = m12;
		rows[1][0] = m21;
...
/* Stores three floating point numbers.  Used as a row of a Matrix33 and to store the data in
...
struct Matrix33
{
	...
	Float3 rows[3]; /*!< The three rows of Float3 structures which hold three floating point numbers
					   each. */
	 ...
	 * \return The Float3 structure for the given row index by reference so it can be read or set
	 * via the bracket operator.
	 */
	NIFLIB_API Float3& operator[](int n) { return rows[n]; }
	NIFLIB_API const Float3& operator[](int n) const { return rows[n]; }
...
struct Matrix44
{
	...
	Float4 rows[4]; /*!< The three rows of Float3 structures which hold three floating point numbers
					   each. */
	 ...
	 * \return The Float4 structure for the given row index by reference so it can be read or set
	 * via the bracket operator.
	 */
	NIFLIB_API Float4& operator[](int n) { return rows[n]; }
	NIFLIB_API Float4 const& operator[](int n) const { return rows[n]; }
...
struct Color3
{
	...
	/*! This constructor can be used to set all values in this structure during initialization
	 * \param[in] r The value to set the red component of this color to.  Should be between 0.0f
	 * and 1.0f. \param[in] g The value to set the green component of this color to. Should be
	 * between 0.0f and 1.0f. \param[in] b The value to set the blue component of this color to.
...
struct Color4
{
	...
	float b; /*!< The blue component of this color.  Should be between 0.0f and 1.0f. */
	float
		a; /*!< The alpha translucency component of this color.  Should be between 0.0f and 1.0f. */
	...
	/*! Copy constructor */
	NIFLIB_API Color4(const Color4& src)
		: r(src.r)
		, g(src.g)
		, b(src.b)
		, a(src.a)
	 ...
	 * and 1.0f. \param[in] g The value to set the green component of this color to. Should be
	 * between 0.0f and 1.0f. \param[in] b The value to set the blue component of this color to.
	 * Should be between 0.0f and 1.0f. \param[in] a The value to set the alpha translucency
	 * component of this color to.  Should be between 0.0f and 1.0f.
	 */
...
/*! Represents a quaternion - a 4D extention of complex numbers used as an alternitive to matrices
...
struct Quaternion
{
	...
	/*! This function returns a Euler Angle representation of the rotation stored in this
	 * quaternion. The angles returned correspond to yaw, pitch, and roll and are in radiens.
...
struct InertiaMatrix
{
	...
	Float4 rows[3]; /*!< The three rows of Float3 structures which hold three floating point numbers
					   each. */
	 ...
	 * \param[in] m The matrix to initialize this one to.
	 */
	NIFLIB_API InertiaMatrix(const InertiaMatrix& m)
	{
		memcpy(rows, m.rows, sizeof(Float4) * _countof(rows));
	}
