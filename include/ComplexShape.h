...
struct TexCoordIndex
{
	...
	NIFLIB_API TexCoordIndex()
		: texCoordSetIndex(CS_NO_INDEX)
		, texCoordIndex(CS_NO_INDEX)
	{}
	/*! Destructor */
	NIFLIB_API ~TexCoordIndex() {}
	/*!
...
struct ComplexPoint
{
	...
	NIFLIB_API ComplexPoint()
		: vertexIndex(CS_NO_INDEX)
		, normalIndex(CS_NO_INDEX)
		, colorIndex(CS_NO_INDEX)
	{}
	/*! Destructor */
	NIFLIB_API ~ComplexPoint() {}
	/*!
...
public:
	 ...
	 * \param use_dismember_partitions Uses BSDismemberSkinInstance with custom partitions for
	 * dismember \return A reference to the root NiAVObject that was created.
	 */
	...
	/*
	 * Sets the name of ComplexShape which will be used when splitting it into NiTriBasedGeom
	 * objects. \param[in] n The new name for the ComplexShape.
	 */
	NIFLIB_API void SetName(const string& n);

	/*
	 * Gets the name of ComplexShape.  This may have been previously created or merged from
	 * NiTriBasedGeom objects which form a split mesh proxy. \return The new name of the
	 * ComplexShape.
	 */
	NIFLIB_API string GetName() const;

	/*
	 * Sets the vertex data that will be used by the ComplexShape, which includes position and skin
	 * weighting information.  The data is referenced by vector index, so repetition of values is
	 * not necessary. \param[in] n The new vertex data.  Will replace any existing data.
	 */
	NIFLIB_API void SetVertices(const vector<WeightedVertex>& n);

	/*
	 * Gets the vertex data from the ComplexShape.  This includes position and skin weighting
	 * information. \return The vertex data stored in the ComplexShape.
	 */
	NIFLIB_API vector<WeightedVertex> GetVertices() const;

	/*
	 * Sets the color data that will be used by the ComplexShape.  The data is referenced by vector
	 * index, so repetition of values is not necessary. \param[in] n The new color data.  Will
	...
	/*
	 * Sets the normal data that will be used by the ComplexShape.  The data is referenced by vector
	 * index, so repetition of values is not necessary. \param[in] n The new normal data.  Will
	 * replace any existing data.
	 */
	...
	/*
	 * Sets the texture coordinate data used by the ComplexShape.  This includes a list of UV sets,
	 * each with a texture type and texture coordiantes.  The coordinate data is referenced by
	 * index, so repetition of values within a particular set is not necessary. \param[in] n The new
	 * texture coordinate sets.  Will replace any existing data.
	 */
	NIFLIB_API void SetTexCoordSets(const vector<TexCoordSet>& n);

	/*
	 * Gets the texture coordinate sets from the ComplexShape.  Each set includes the type of
	 * texture and the texture coordinates. \return The texture coodinate sets stored in the
	 * ComplexShape.
	 */
	NIFLIB_API vector<TexCoordSet> GetTexCoordSets() const;

	/*
	 * Sets the faces used by the ComplexShape.  Each face references 3 or more vertex positions
	 * (triangles are not required), each of which can reference color, normal, and texture,
	 * information.  Each face also references a property group which defines the material and any
	 * other per-face attributes. \param[in] n The new face data.  Will replace any existing data.
	 */
	NIFLIB_API void SetFaces(const vector<ComplexFace>& n);

	/*
	 * Gets the face data from the ComplexShape. Each face references 3 or more vertex positions
	 * (triangles are not required), each of which can reference color, normal, and texture,
	 * information.  Each face also references a property group which defines the material and any
	 * other per-face attributes. \return The face data stored in the ComplexShape.
	 */
	NIFLIB_API vector<ComplexFace> GetFaces() const;

	/*
	 * Sets the property groups used by the Complex Shape.  Each group of NiProperty values can be
	 * assigned by vector index to a face in the ComplexShape by index, allowing for material and
	 * other properties to be specified on a per-face basis.  If the ComplexShape is split, each
	 * property group that is used by faces in the mesh will result in a separate NiTriBasedGeom
	 * with the specified propreties attached. \param[in] n The new property groups.  Will replace
	 * any existing data.
	 */
	NIFLIB_API void SetPropGroups(const vector<vector<Ref<NiProperty>>>& n);

	/*
	 * Gets the property groups from the ComplexShape.  Each group of NiProperty values can be
	 * assigned by vector index to a face in the ComplexShape by index, allowing for material and
	 * other properties to be specified on a per-face basis.  If the ComplexShape is split, each
	 * property group that is used by faces in the mesh will result in a separate NiTriBasedGeom
	 * with the specified propreties attached. \return The property groups stored in the
	 * ComplexShape.
	 */
	NIFLIB_API vector<vector<Ref<NiProperty>>> GetPropGroups() const;

	/*
	 * Sets the skin influences used by the Complex Shape.  These are the NiNode objects which cause
	 * deformations in skin meshes.  They are referenced in the vertex data by vector index.
	...
	/*
	 * Gets the skin influences from the ComplexShape.  These are the NiNode objects which cause
	 * deformations in skin meshes.  They are referenced in the vertex data by vector index. \return
	 * The skin influences referenced by the ComplexShape.
	 */
	 ...
	 * \param[in] The new association meaning that the face at the position of the index corresponds
	 * to the body part group given by the value at the position of the index of the face
	 */
