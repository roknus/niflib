...
template<>
void NifStream(Key<IndexString>& key, istream& file, const NifInfo& info, KeyType type);
template<>
void NifStream(Key<IndexString> const& key, ostream& file, const NifInfo& info, KeyType type);
...
public:
	static NifInfo* getInfo(ios_base& str) { return (NifInfo*)str.pword(infoIdx); }
};
...
public:
	static Header* getInfo(ios_base& str) { return (Header*)str.pword(infoIdx); }
};
